...
mod tests {
    ...
    mod store_with_table_wrappers {
        use super::*;
        use crate::{impl_redb_uuid, utils::UuidV7};

        #[derive(Clone, Copy, Debug, PartialEq, Eq)]
        struct TestId(UuidV7);
        impl_redb_uuid!(TestId);

        const UUID_TABLE: UuidTable<TestId, &str> =
            UuidTable::new("uuid_table");
        const GENERIC_TABLE: Table<&str, &str> = Table::new("generic_table");
        const PATH_TABLE: PathTable<&str> = PathTable::new("path_table");
        ...
        fn store_with_uuid_table() -> Result<(), DbError> {
            let temp_dir = tempfile::TempDir::new().unwrap();
            let db_path = temp_dir.path().join("test.db");
...

            {
                ...
                store.write(|tx| {
                    let mut table =
                        tx.inner.open_table(UUID_TABLE.definition())?;
                    table.insert(id1, "value1")?;
                    table.insert(id2, "value2")?;
                    Ok(())
                })?;
            };

            {
                let store = Store::open(&db_path)?;
                store.read(|tx| {
                    let table = tx.inner.open_table(UUID_TABLE.definition())?;
                    let v1 = table.get(id1)?.expect("value1 should exist");
                    assert_eq!(v1.value(), "value1");
                    let v2 = table.get(id2)?.expect("value2 should exist");
        ...
        fn store_with_generic_table() -> Result<(), DbError> {
            let temp_dir = tempfile::TempDir::new().unwrap();
            let db_path = temp_dir.path().join("test.db");

            {
                ...
                store.write(|tx| {
                    let mut table =
                        tx.inner.open_table(GENERIC_TABLE.definition())?;
...

            {
                ...
                store.read(|tx| {
                    let table =
                        tx.inner.open_table(GENERIC_TABLE.definition())?;
                    let v1 = table.get("key1")?.expect("val1 should exist");
                    assert_eq!(v1.value(), "val1");
                    Ok(())
                })?;
            };
        ...
        fn store_with_path_table() -> Result<(), DbError> {
            let temp_dir = tempfile::TempDir::new().unwrap();
            let db_path = temp_dir.path().join("test.db");

            {
                ...
                store.write(|tx| {
                    let mut table =
                        tx.inner.open_table(PATH_TABLE.definition())?;
                    table.insert("/path/to/file1", "metadata1")?;
                    table.insert("/path/to/file2", "metadata2")?;
                    Ok(())
                })?;
            };

            {
                let store = Store::open(&db_path)?;
                store.read(|tx| {
                    let table = tx.inner.open_table(PATH_TABLE.definition())?;
                    let v1 = table
                        .get("/path/to/file1")?
                        .expect("metadata1 should exist");
                    assert_eq!(v1.value(), "metadata1");
                    Ok(())
                })?;
            };
        ...
        fn store_with_multiple_tables_in_transaction() -> Result<(), DbError> {
            let temp_dir = tempfile::TempDir::new().unwrap();
            let db_path = temp_dir.path().join("test.db");
...

            {
                ...
                store.write(|tx| {
                    ...
                    uuid_tbl.insert(id, "uuid_value")?;
                    generic_tbl.insert("key", "generic_value")?;
                    path_tbl.insert("/test/path", "path_value")?;
...

            {
                ...
                store.read(|tx| {
                    ...
                    let uuid_val =
                        uuid_tbl.get(id)?.expect("uuid_value should exist");
                    assert_eq!(uuid_val.value(), "uuid_value");

                    let generic_val = generic_tbl
                        .get("key")?
                        .expect("generic_value should exist");
                    assert_eq!(generic_val.value(), "generic_value");

                    let path_val = path_tbl
                        .get("/test/path")?
                        .expect("path_value should exist");
                    assert_eq!(path_val.value(), "path_value");
