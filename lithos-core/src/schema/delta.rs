...
impl<'data> PropertyDeltaEngine<'data, RawProperty> {
    ...
    // TODO(.scratch/base-schema/): consolidate `diff_schema` and
    // `diff_schema_without_bank` into a single method. The two paths share the
    // same `compute_change_set` core and differ only in how `$ref` upserts are
    // handled (expand via `RefExpander` vs skip with a warning). One approach
    // is to accept an `Option<&RefExpander>` and branch internally; another is
    // to introduce a strategy type or a small `RefStrategy` enum
    // (`Expand(&RefExpander)` | `SkipWithWarning`) so the call site remains
    // explicit without duplicating the hash-comparison logic.

    /// Computes a schema property delta without a [`RefExpander`].
    ///
    /// Used when no [`PropertyBank`] is available. Changed `$ref` entries are
    /// skipped and logged; only inline property changes are reflected in the
    /// returned delta. Removals are computed from the hash comparison
    /// regardless of property type.
    ///
    /// # Errors
    ///
    /// Returns [`SchemaLoaderError`] when changed inline entries cannot be
    /// converted into a validated [`PropertyMap`].
    #[inline]
    pub(crate) fn diff_schema_without_bank(
        &self,
    ) -> Result<PropertyDelta, SchemaLoaderError> {
        let (raw_upserts, removals, _current_hashes) =
            self.compute_change_set();
        ...
        if ref_count > 0 {
            tracing::warn!(
                ref_count,
                "property bank missing; treating schema $ref changes as \
                 unexpanded"
            );
        }

        let upserts = if inline_map.is_empty() {
            PropertyMap::new()
        } else {
            PropertyMap::try_from(inline_map)
                .map_err(SchemaLoaderError::Resolution)?
        };

        Ok(PropertyDelta::new(upserts, removals))
    }
}
