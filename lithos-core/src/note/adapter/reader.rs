...
#[expect(
    ...
    reason = "Legacy parser retained until Phase 6 cleanup"
)]
#[expect(
    clippy::wildcard_imports,
    reason = "Legacy parser keeps super::* for readability"
)]
#[expect(dead_code, reason = "Legacy parser retained until Phase 6 cleanup")]
mod legacy_parser {
    use super::*;
    ...
    impl<'config, 'source> ParseState<'config, 'source> {
        ...
        fn handle_start_tag(
            ...
            match tag {
                ...
                CmarkTag::CodeBlock(_) => {
                    self.code_block_depth =
                        self.code_block_depth.saturating_add(1);
                }
                CmarkTag::Paragraph
        ...
        fn handle_end_tag(&mut self, tag_end: TagEnd) -> Result<(), NoteError> {
            ...
            match tag_end {
                ...
                TagEnd::CodeBlock => {
                    self.code_block_depth =
                        self.code_block_depth.saturating_sub(1);
                    close_block = true;
                }
                TagEnd::Paragraph
                | TagEnd::BlockQuote(_)
    ...
    impl ItemCollector {
        ...
        fn status_symbol_from_marker(
            ...
            // pulldown-cmark only exposes a checked boolean, so custom symbols
            // in the source cannot be recovered here.
            ...
            let (target_str, anchor) = if state.is_external {
                ...
                let anchor =
                    if let Some(block_ref) = anchor_part.strip_prefix("#^") {
                        ...
                        let anchor_part =
                            anchor_part.strip_prefix('#').unwrap_or("");
