...
mod tests {
    ...
    mod fixtures {
        ...
        pub fn scanned_template_for_path(path_key: PathKey) -> ScannedTemplate {
            ...
            let file_path = crate::fs::FilePath::try_new(file_path).unwrap();
            let metadata = crate::fs::metadata::FileMetadata::new(
                crate::fs::metadata::FsTimes::new(None, None),
                7,
                false,
            );
            let file = crate::fs::entry::FileNode::new(file_path, metadata);
    ...
    mod check_batch_existence {
        ...
        use crate::{
            ...
            template::{
                ...
                processor::{
                    Corrupted, DiscoveredCacheState, DiscoveredTemplate,
                    Missing, Present,
                },
                views::RawTemplateView,
            },
    ...
    mod load {
        ...
        use super::*;
        use crate::support::content_hash::{
            Blake3Hash, HasContentHash, HashInput,
        };
