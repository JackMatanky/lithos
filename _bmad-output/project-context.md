...
- **Naming Convention:**
  ...
  - Repository traits: Prefer short, qualified names: `<Context>::Repository` (e.g., `schema::Repository`, `note::Repository`)
...
- **Zero-Copy Access:** Repository methods use closure-based access patterns (`with_archived`) to enable zero-copy reads without leaking transaction lifetimes.
...
- **Repository Trait Naming:**
  - **Repository Traits:** Short, qualified names: `<Context>::Repository` (e.g., `schema::Repository`, `note::Repository`).
  - **Implementations:** Descriptive suffixes: `RedbRepository`, `InMemoryRepository`, `FakeRepository`.
  - **Type Aliases:** Use module-qualified usage for clarity: `schema::RedbRepository`, `note::InMemoryRepository`.
