...
**Pattern Consistency:**
- **Unified Storage Traits:** Each context defines a single `Storage` trait combining read and write operations (no CQRS split), achieving testability via multiple implementations (Redb, InMemory, Fake)
...
**Structure Alignment:**
The **Single-Crate Architecture** aligns perfectly with zero-copy performance requirements while module boundaries and unified Storage traits maintain logical separation and testability.
...
- Each context defines single `Storage` trait (e.g., `schema::Storage`, `note::Storage`)
...
- Test substitution enabled via trait implementations (`RedbStorage`, `InMemoryStorage`, `FakeStorage`)
