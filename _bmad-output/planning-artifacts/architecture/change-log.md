...
| 2025-11-01 | 0.6.1   | Updated SchemaValidator.ValidateAll() signature to remove unused PropertyBank parameter, added performance optimizations (validation caching, regex compilation, context propagation) | James |
| 2025-11-01 | 0.6.2   | DDD Architecture Alignment: Property transformed to entity with hash-based ID, PropertyRef/IProperty removed from domain, SchemaValidator/SchemaResolver moved to adapter layer, split into PropertyDereferencer/SchemaExtender, SchemaEngine simplified to pure orchestration | Sarah |
| 2025-10-24 | 0.5.10  | Added moby/sys/atomicwriter to tech stack for atomic file writes | Winston |
| 2025-10-24 | 0.5.11  | Updated components.md: removed old orchestrator, updated domain services (TemplateEngine, FrontmatterService, SchemaEngine with generics, VaultIndexer, QueryService), updated API ports (CLIPort), updated SPI ports (CacheWriter/Reader, SchemaPort, TemplatePort, PromptPort/FinderPort split, ConfigPort, SchemaRegistryPort), removed filesystem ports per YAGNI, updated adapters (split JSONCache adapters, SchemaLoaderAdapter, TemplateLoaderAdapter, PromptUIAdapter, FuzzyFinderAdapter, ViperAdapter, SchemaRegistryAdapter, CobraCLIAdapter) | Winston |
...
| 2025-10-26 | 0.6.4   | Updated components.md: Added CLICommander as proper use case orchestrator, redesigned CLIPort with CommandPort callback interface for hexagonal architecture, updated CobraCLIAdapter with SRP decomposition pattern | Winston |
...
| 2025-10-26 | 0.6.8   | Added VaultReaderPort and VaultWriterPort with CQRS pattern for vault operations, added VaultReaderAdapter and VaultWriterAdapter, renamed CacheWriterPort.Write to Persist for consistency, updated VaultIndexer and CLICommander dependencies, updated component diagrams | Winston |
| 2025-11-09 | 1.0.0   | Epic 3 Course Correction: Renamed GoldmarkParserAdapter to MarkdownParserAdapter, removed IndexMaintenanceService (consolidated into VaultIndexer), renamed CommandOrchestrator to CLICommander, updated all components for hybrid storage architecture (BoltDB + SQLite), added EventBusAdapter for event-driven architecture, added BoltDBReaderAdapter and SQLiteReaderAdapter for MetadataQueryPort | Sarah |
