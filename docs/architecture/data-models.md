...
## CacheUnitOfWork (Application Model)

**Purpose:** Coordinate dual-write transactions across BoltDB (hot cache) and SQLite (deep storage) so VaultIndexer can treat the hybrid layer as a single logical writer (Story 3.22).

**Location:** `internal/app/vault/unit_of_work.go`

**Key Fields:**

- `boltWriter CacheWriterPort` – Persists primary data and indices inside BoltDB.
- `sqliteWriter CacheWriterPort` – Persists metadata rows/views inside SQLite.
- `operations []operation` – In-memory staging buffer holding pending writes/deletes plus a captured `indexTime` shared by both stores.
- `mu sync.Mutex` – Ensures transaction isolation while staging operations.
...
**Workflow:**
1. `Begin()` opens BoltDB (`bolt.Tx`) and SQLite (`sql.Tx`) transactions.
2. `AddWrite`/`AddDelete` append operations to the staging slice without touching storage.
3. `Commit(ctx)` replays staged operations against BoltDB first, then SQLite, propagating the same `indexTime` so `FileDatesDTO` and `indexed_time` remain aligned.
4. Failures trigger compensating rollbacks (BoltDB rollback before SQLite writes; SQLite rollback plus BoltDB cleanup when the second phase fails).
5. `Rollback(ctx)` is idempotent and clears staged operations so callers can safely retry.

**Why it matters:** Prevents split-brain cache state (note persisted to BoltDB but not SQLite) and centralizes transactional logic, making it easy to plug in future storage providers.

## SchemaViewDefinition (Adapter Model)

**Purpose:** Describe the SQL views and indexes generated by Story 3.21 so `MetadataQueryPort` can issue O(1) frontmatter queries via SQLite.

**Location:** `internal/adapters/spi/cache/sqlite/views.go`
...
**Key Concepts:**
- **Naming convention:** `v_{schema_name}_notes` mirrors Schema IDs, simplifying MetadataQueryPort routing.
- **Typed projection:** Columns map to PropertySpec types (string/int/real/bool) so SQLite can leverage native indexes instead of JSON scans.
- **Automatic regeneration:** View + index DDL is rebuilt whenever schemas change, ensuring adapters always reflect the active PropertyBank definitions.
- **Performance contract:** Indexed queries must complete <50 ms for 1000 notes; benchmark outputs feed into `docs/architecture/performance-guide.md`.

**Usage:** SQLite adapters call `GenerateSchemaView(schema)` during initialization, install missing indexes, and expose the resulting view via MetadataQueryPort (`FileClassQuery`, `FrontmatterQuery`, `TagQuery`).
...
**Status:** ACTIVE - Epic 3 implements event-driven architecture (Story 3.30). Event bus with domain events replaces direct service dependencies to eliminate god-object pattern (CLICommander, VaultIndexer).
