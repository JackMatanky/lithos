...
`traces-core`: Discovery → Config → Indexer → routing. Wiring these steps
...
Three candidate locations were considered: inside `traces-cli` (the executable
adapter), inside a new `main.rs` added to `traces-core`, or inside a dedicated
`crates/app` module.
...
- `traces-core` remains a library crate with no `main.rs`.
- `crates/app` composes core ports and adapters and exposes typed
  execution flows (e.g. `run_index`) to executable adapters.
- `traces-cli` remains a thin executable adapter: it parses user intent,
...
`traces-cli` constructs all adapters and wires the execution flow directly.
...
### Alternative 2: `main.rs` in `traces-core`

Add `[[bin]]` to `traces-core` and embed composition logic in `main.rs`.

- **Pros**: Slightly fewer crate boundaries.
- **Cons**: Turns `traces-core` into a mixed library-binary crate, which
...
- The hexagonal architecture guide (`docs/refs/rust/howtocodeit-hexagonal`)
  ...
  to the caller.
- Every other context in `traces-core` already follows the pattern of
...
- **Positive**: `traces-cli` can remain a thin adapter focused on CLI
...
- **Negative**: `traces-core` now has a module (`app`) that is not a domain
