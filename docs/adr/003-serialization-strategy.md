...
- **Public Zero-Copy APIs**: Use the generic `rkyv::Archived<T>` for public zero-copy APIs (e.g., the `Storage` trait's `with_archived` method). This ensures consumers don't need to know about generated internal type names.
