...
1. **Feature-Gated Serde**: `serde` should generally not be used in the domain. It pollutes domain logic and slows down compilation. It should only be used behind feature flags specifically for CLI/LSP JSON output needs, and even then sparingly. When required, domain entities use `#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]`
...
- **Storage Abstraction**: Unified `Repository` traits use GATs (Generic Associated Types) to expose `Archived<'a>` views without leaking transaction lifetimes
...
When working with `rkyv`'s zero-copy types, we follow specific rules for where and how `Archived*` types are exposed:

- **Public Zero-Copy APIs**: Use the generic `rkyv::Archived<T>` for public zero-copy APIs (e.g., the `Repository` trait's `with_archived` method). This ensures consumers don't need to know about generated internal type names.
- **Internal Accessors**: Local `Archived*` types (e.g., `ArchivedTag`, `ArchivedSchema`) generated by the `#[derive(Archive)]` macro are only used *internally* within a module to add small, safe accessors for private fields on the archived representations without deserializing.
- **No Leaking**: These generated `Archived*` types must not leak into public signatures outside their defining module.
