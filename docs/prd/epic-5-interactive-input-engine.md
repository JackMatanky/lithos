...
This epic delivers the interactive ports, adapters, TemplateEngine helpers, and CLI wiring defined in architecture v0.6.8. Completing it lets Lithos gather user input for templates and discover templates interactively through consistent PromptPort and FinderPort abstractions. Stories proceed from SPI contracts to adapter implementations and finally to TemplateEngine and CommandOrchestrator integration. This epic builds on the schema-driven lookups from Epic 4 and adds interactive capabilities.
...
5. `golangci-lint run ./internal/ports/spi` succeeds.
...
5. Unit tests cover successful prompt/suggester interactions, validation failures, cancellation, and non-interactive fallback.
5. `golangci-lint run ./internal/adapters/spi/interactive` and `go test ./internal/adapters/spi/interactive` succeed.

---

## Story 5.3 FinderPort & Fuzzy Adapter
...
5. Unit tests simulate selection success, cancellation, non-interactive mode, and ensure logging meets coding standards.
5. `golangci-lint run ./internal/adapters/spi/interactive` and `go test ./internal/adapters/spi/interactive` succeed.
...
5. Unit tests with fake ports verify helper behaviour, path context handling, error propagation, and ensure interactive helpers respect non-interactive mode.
5. `golangci-lint run ./internal/app/template` and `go test ./internal/app/template` succeed.

---

## Story 5.5 CLI Find Command
