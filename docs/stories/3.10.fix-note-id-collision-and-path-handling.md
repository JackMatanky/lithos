...
4. QueryService PathQuery and BasenameQuery methods must work correctly with the new NoteID scheme
...
- Implemented BasenameQuery query method with proper basename extraction
...
- Modified: `internal/app/query/service.go` - Added queryByField helper, BasenameQuery method, updated RefreshFromCache
- Modified: `internal/app/query/service_test.go` - Updated tests for path-based NoteIDs and BasenameQuery
