...
3. QueryService updated to support frontmatter-based queries: FrontmatterQuery(field, value) method enables lookup by frontmatter field values from indexed Notes.
...
- [x] Task 3: Implement QueryService frontmatter queries (AC: 3)
  - [x] RED: Write failing tests for FrontmatterQuery method
    - [x] Write test case expecting FrontmatterQuery(field, value) method
  ...
  - [x] GREEN: Implement FrontmatterQuery method
    - [x] Add FrontmatterQuery(field string, value interface{}) ([]Note, error) method
  ...
  - [x] REFACTOR:
    ...
    - [x] Add comprehensive GoDoc for FrontmatterQuery method
...
```go
// FrontmatterQuery enables queries like:
...
3. **QueryService Enhancement**: Added FrontmatterQuery method for frontmatter-based queries
   - Implemented byFrontmatter index: map[string]map[interface{}][]domain.Note
   - Added FrontmatterQuery(ctx, field, value) method with thread-safe access
   - Updated RefreshFromCache to populate frontmatter index
   - Skips non-comparable frontmatter values (arrays/maps) and only indexes normalized primitives to avoid runtime panics
...
- `internal/app/query/service.go` - Added FrontmatterQuery method and frontmatter indexing
- `internal/app/query/service_test.go` - Added comprehensive tests for FrontmatterQuery
...
**QueryService Enhancement**: Clean implementation of FrontmatterQuery method with thread-safe access and efficient nested map indexing. Good separation of concerns between indexing and querying.
...
| 2025-11-01 | 1.1     | Implemented FrontmatterService integration - VaultIndexer enhanced with frontmatter processing, QueryService enhanced with FrontmatterQuery method. All ACs completed. | James (Dev) |
