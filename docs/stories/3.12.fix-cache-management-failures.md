...
- Implemented deletion reconciliation in Refresh method by adding reconcileDeletions helper function that compares current vault NoteIDs against cached NoteIDs and deletes orphans
...
- Consolidated ensureCacheDir implementations by creating shared utils.EnsureCacheDir function in internal/shared/utils/cache.go
- Updated json_writer.go and helper.go to use the shared function, removed duplicate implementations
...
- internal/shared/utils/cache.go: New shared EnsureCacheDir function
- internal/adapters/spi/cache/helper.go: Updated to use shared EnsureCacheDir
- internal/adapters/spi/cache/json_writer.go: Updated to use shared EnsureCacheDir, removed duplicate method and unused constant
