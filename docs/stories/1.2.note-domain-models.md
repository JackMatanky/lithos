...
- [ ] Task 1: Implement NoteID model (AC: 1.2.1)
  - [ ] RED: Write failing test for NewNoteID constructor
  ...
  - [ ] RED: Write failing test for String() method
    ...
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] RED: Write failing test for using NoteID as map key
    - [ ] Create test using NoteID as map[NoteID]string key
    - [ ] Verify compilation and test behavior
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] GREEN: Implement minimal code to pass tests
    - [ ] Implement `type NoteID string`
    - [ ] Implement `NewNoteID(value string) NoteID` constructor
  ...
  - [ ] REFACTOR:
    - [ ] Decompose into SRP components (minimal - already simple types):
      - [ ] Verify NoteID type has single responsibility (opaque identifier)
      - [ ] Verify constructor has single responsibility (creation)
      - [ ] Verify String() method has single responsibility (string conversion)
    - [ ] Review naming: type NoteID (clear), NewNoteID (idiomatic), String() (standard Go interface)
    - [ ] Add comprehensive GoDoc comments:
      - [ ] Add package comment at top of note.go if not present
      - [ ] Add GoDoc for NoteID type explaining opaque identifier concept
      - [ ] Add GoDoc for NewNoteID constructor
    ...
    - [ ] Verify test coverage >90% for NoteID
  - [ ] Linting checkpoint:
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL warnings (no nolint unless absolutely necessary)
    - [ ] Document any unavoidable nolint with clear justification

- [ ] Task 2: Implement Frontmatter model (AC: 1.2.2)
  - [ ] RED: Write failing test for NewFrontmatter constructor
    - [ ] Write test case in `internal/domain/note_test.go`
    - [ ] Verify test fails with expected error message
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] RED: Write failing test for FileClass extraction from Fields
    - [ ] Write test with Fields["fileClass"] = "contact"
    - [ ] Verify FileClass field populated correctly
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] RED: Write failing test for FileClass empty when not present
    - [ ] Write test with Fields missing "fileClass" key
    - [ ] Verify FileClass is empty string
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] RED: Write failing test for SchemaName() method
    - [ ] Write test verifying SchemaName() returns FileClass
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] GREEN: Implement minimal code to pass tests
    - [ ] Implement Frontmatter struct with FileClass and Fields
    - [ ] Implement `extractFileClass(fields) string` helper
    - [ ] Implement `NewFrontmatter(fields) Frontmatter` constructor
    - [ ] Implement `SchemaName() string` method
    - [ ] Run `go test ./internal/domain` and verify all tests pass
    - [ ] Verify no other tests broken
  - [ ] REFACTOR:
    - [ ] Decompose into SRP components:
      - [ ] Verify Frontmatter struct has single responsibility (hold frontmatter data)
      - [ ] Verify extractFileClass helper has single responsibility (extract fileClass from fields map)
      - [ ] Verify NewFrontmatter constructor has single responsibility (create Frontmatter with extracted FileClass)
      - [ ] Verify SchemaName() has single responsibility (return schema name)
      - [ ] Consider: If extractFileClass grows >10 lines, extract type assertion logic to validateFileClassType()
    - [ ] Review naming:
      - [ ] Frontmatter (clear domain concept)
      - [ ] FileClass (matches architecture spec)
      - [ ] Fields (clear: frontmatter field data)
      - [ ] extractFileClass (descriptive: what it does)
      - [ ] SchemaName (clear interface method)
    - [ ] Add comprehensive GoDoc comments:
      - [ ] Add GoDoc for Frontmatter struct explaining anemic model design
      - [ ] Add GoDoc for FileClass field (schema reference)
      - [ ] Add GoDoc for Fields field (complete YAML data, preserves unknown fields per FR6)
      - [ ] Add GoDoc for NewFrontmatter constructor
      - [ ] Add GoDoc for extractFileClass helper (private, but comment for clarity)
      - [ ] Add GoDoc for SchemaName() method
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL linter warnings without using nolint
    - [ ] Run `go test ./internal/domain` to verify refactoring didn't break tests
    - [ ] Verify test coverage >90% for Frontmatter
  - [ ] Linting checkpoint:
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL warnings (no nolint unless absolutely necessary)
    - [ ] Document any unavoidable nolint with clear justification

- [ ] Task 3: Implement Note model (AC: 1.2.3)
...
- `internal/adapters/spi/file_metadata.go` - FileMetadata model (Story 1.9)
