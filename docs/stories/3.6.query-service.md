...
1. ✅ PRESERVE: QueryService implements same public interface (`IDQuery`, `PathQuery`, `FileClassQuery`, `FrontmatterQuery`, `RefreshFromCache`)

2. 🔄 ENHANCE: Smart query routing based on query type:
   - **Hot Path Queries** → BoltDB: `PathQuery`, `BasenameQuery`, `AliasQuery` for sub-millisecond performance
   - **Complex Queries** → SQLite: `FrontmatterQuery`, complex filtering for query optimization
...
- [x] Task 2: Implement query methods (AC: 1, 2)
   - [x] RED: Write failing tests for all query methods
     - [x] Write test for IDQuery() returning note for existing NoteID
     - [x] Write test for IDQuery() returning ResourceError for missing NoteID
     - [x] Write test for PathQuery() returning note for existing path
     - [x] Write test for PathQuery() returning ResourceError for missing path
     - [x] Write test for FileClassQuery() returning all notes matching schema
     - [x] Write test for FileClassQuery() returning empty slice for non-matching schema
     - [x] Note: FrontmatterQuery tests added in Story 3.8

    - [x] Verify tests fail (methods not implemented)
    - [x] Run `go test ./internal/app/query` and confirm failures
  - [x] GREEN: Implement IDQuery() method
  ...
  - [x] REFACTOR:
    ...
    - [x] Review naming: IDQuery, PathQuery, FileClassQuery (clear, descriptive method names)
     - [x] Add comprehensive GoDoc comments:
...
- [ ] Task 8: Implement smart query routing architecture (Enhanced AC: 1-2)
  ...
  - [ ] Route hot path queries (PathQuery, BasenameQuery, AliasQuery) to BoltDB
  - [ ] Route complex queries (FrontmatterQuery) to SQLite
...
| 2025-10-31 | 1.4     | Task 2 complete: Implemented IDQuery, PathQuery, FileClassQuery methods with thread-safety and error handling | James (Dev)        |
...
2. **Smart Routing Logic**: IDQuery, PathQuery, FileClassQuery, BasenameQuery use BoltDB for O(1) lookups; FrontmatterQuery uses SQLite for complex field queries
...
- ✅ Performance optimized with O(1) lookups for IDQuery/PathQuery, O(log n) for FileClassQuery
...
- Architecture specifies `PathQuery(ctx context.Context, path string) ([]Note, error)` but implementation returns `(Note, error)`
...
- O(1) lookups for primary operations (IDQuery, PathQuery)
