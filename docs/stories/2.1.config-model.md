...
- [ ] Task 5: Implement Schema struct (AC: 2.1.12-2.1.15)
  - [ ] RED: Write failing test for NewSchema with valid schema
    - [ ] Write test case in `internal/domain/schema_test.go`
    - [ ] Create Schema with Name, Properties
    - [ ] Verify test fails (Schema type doesn't exist yet)
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] RED: Write failing test for Schema.Validate() with missing name
    - [ ] Write test expecting validation error for empty name
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] RED: Write failing test for excludes without extends
    - [ ] Write test expecting validation error when Excludes populated but Extends empty
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] GREEN: Implement minimal code to pass tests
    - [ ] Create `internal/domain/schema.go`
    - [ ] Implement Schema struct with four fields and JSON/YAML tags
    - [ ] Implement NewSchema() with defensive slice copies
    - [ ] Implement Validate() method with name and excludes/extends checks
    - [ ] Run `go test ./internal/domain` and verify all tests pass
    - [ ] Verify no other tests broken
  - [ ] REFACTOR:
    - [ ] Decompose Validate() into SRP components:
      - [ ] Extract validateName() private method (name validation only)
      - [ ] Extract validateExcludesConstraint() private method (excludes/extends constraint only)
      - [ ] Keep Validate() as orchestrator that delegates to private methods
      - [ ] Verify each method has single responsibility
      - [ ] Verify Schema struct has single responsibility (hold schema definition)
      - [ ] Verify NewSchema has single responsibility (create Schema with immutability)
    - [ ] Review naming:
      - [ ] Schema (clear), NewSchema (idiomatic), Validate (Go convention)
      - [ ] Private methods: validateName, validateExcludesConstraint (descriptive, verb-based)
    - [ ] Add comprehensive GoDoc comments:
      - [ ] Add GoDoc for Schema struct explaining purpose and immutability
      - [ ] Add GoDoc for each field explaining purpose
      - [ ] Add GoDoc for NewSchema explaining defensive copies
      - [ ] Add GoDoc for Validate explaining orchestration of validation
      - [ ] Add GoDoc for each private validation method explaining single responsibility
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL linter warnings without using nolint
    - [ ] Run `go test ./internal/domain` to verify refactoring didn't break tests
    - [ ] Verify test coverage >90% for Schema
  - [ ] Linting checkpoint:
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL warnings (no nolint unless absolutely necessary)
    - [ ] Document any unavoidable nolint with clear justification

- [ ] Task 6: Implement Schema validation delegation (AC: 2.1.14)
  - [ ] RED: Write failing test for Schema.Validate() delegating to Property.Validate()
    - [ ] Create Schema with invalid Property
    - [ ] Expect validation error from property validation
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] GREEN: Implement property validation delegation
    - [ ] Add validateProperties(ctx) private method to Schema
    - [ ] Loop through Properties slice in validateProperties()
    - [ ] Call Validate(ctx) on each Property
    - [ ] Aggregate errors using errors.Join
    - [ ] Call validateProperties(ctx) from Validate() orchestrator
    - [ ] Run `go test ./internal/domain` and verify test passes
  - [ ] REFACTOR:
    - [ ] Verify validateProperties() has single responsibility (property delegation only)
    - [ ] Ensure error messages include property context
    - [ ] Verify errors conform to error-handling-strategy.md
    - [ ] Add GoDoc for validateProperties() explaining delegation and aggregation
    - [ ] Verify orchestration: Validate() → validateName() → validateExcludesConstraint() → validateProperties()
  - [ ] Linting checkpoint:
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL warnings

- [ ] Task 7: Enforce Schema value object semantics (AC: 2.1.16-2.1.17)
  - [ ] RED: Write test verifying immutability (modifying constructor args doesn't affect Schema)
    - [ ] Create Schema with Properties slice
    - [ ] Modify original slice after construction
    - [ ] Verify Schema internal state unchanged
    - [ ] Run `go test ./internal/domain` and confirm behavior
  - [ ] Verify documentation explains immutability and defensive copies
  - [ ] Reference architecture docs in comments
  - [ ] Linting checkpoint:
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL warnings

- [ ] Task 8: Write comprehensive Schema unit tests (AC: 2.1.18-2.1.20)
  - [ ] RED: Write failing test for JSON/YAML marshaling round-trip
    - [ ] Write test marshaling Schema to JSON and back
    - [ ] Write test marshaling Schema to YAML and back
    - [ ] Verify round-trip preserves all fields
    - [ ] Run `go test ./internal/domain` and confirm failure
  - [ ] GREEN: Ensure JSON/YAML tags support marshaling
    - [ ] Verify tags on all fields
    - [ ] Run `go test ./internal/domain` and verify test passes
  - [ ] Add test for Schema with Extends and Excludes
  - [ ] Add test for Schema with multiple Properties
  - [ ] Run golangci-lint and fix any issues
  - [ ] Run tests and verify all pass
  - [ ] Linting checkpoint:
    - [ ] Run `golangci-lint run --fix internal/domain`
...
- [ ] Task 9: Schema quality gates and commit (AC: 2.1.21-2.1.22)
  - [ ] Run `go test ./internal/domain` and verify 100% pass
  - [ ] Run `golangci-lint run --fix internal/domain` and fix any issues
  - [ ] Verify test coverage >90% for Schema
  - [ ] Add documentation about SchemaPort loading and SchemaValidator
  - [ ] Review all GoDoc comments
  - [ ] Commit with message: `feat(domain): add Schema domain model with validation`
  - [ ] Linting checkpoint:
    - [ ] Run `golangci-lint run --fix internal/domain`
    - [ ] Fix ALL warnings (no nolint unless absolutely necessary)
...
**Schema Model (🚧 IN PROGRESS):**
- Tasks 5-9 pending implementation
- Schema struct with Name, Extends, Excludes, Properties needs to be created
- Schema.Validate() method with structural validation needs implementation
- Unit tests for Schema model need to be written
- Integration with Property model required (Story 2.2)
...
**Pending:**
- to create: internal/domain/schema.go (Schema struct, NewSchema constructor, Validate method)
- to create: internal/domain/schema_test.go (comprehensive unit tests for Schema)
...
**Config Model:** Gate: PASS → docs/qa/gates/2.1-config-model.yml

**Schema Model:** Gate: PENDING → Awaiting completion of Tasks 5-9

### Recommended Status

**Story Status:** 🚧 IN PROGRESS
- Config Model: ✅ Complete - All requirements met, tests pass, quality standards achieved
- Schema Model: ⏳ Pending - Tasks 5-9 require implementation before story can be marked Done
