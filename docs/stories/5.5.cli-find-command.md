...
- 5.5.1: Add `FindTemplates` method to `internal/app/command/orchestrator.go`:
...
- 5.5.2: Implement error handling for FindTemplates:
...
- 5.5.3: Add `find` command to `internal/adapters/api/cobra/cli.go`:
  - Command: `lithos find`
...
- 5.5.4: Implement Template type to satisfy FinderItem interface:
...
- 5.5.5: Handle non-interactive environments gracefully:
...
- 5.5.6: Implement output formatting for find command:
...
- 5.5.7: Implement error handling per `docs/architecture/error-handling-strategy.md`:
...
- 5.5.8: Handle user cancellation (Ctrl+C) gracefully:
...
- 5.5.9: Follow coding standards for logging:
...
- 5.5.10: Create `tests/integration/cli_find_test.go` with tests for:
...
- 5.5.11: Create `tests/e2e/find_command_test.go` with tests for:
...
- 5.5.12: Run `golangci-lint run ./internal/app/command` - all checks pass

- 5.5.13: Run `golangci-lint run ./internal/adapters/api/cobra` - all checks pass

- 5.5.14: Run `go test ./internal/app/command` - all tests pass with >80% coverage

- 5.5.15: Run `go test ./internal/adapters/api/cobra` - all tests pass with >80% coverage

- 5.5.16: Run `go test ./tests/integration` - integration tests pass

- 5.5.17: Run `go test ./tests/e2e` - e2e tests pass

- 5.5.18: Verify FR2 requirements satisfied (CLI command documented)

- 5.5.19: Verify FR10 requirements satisfied (interactive UX with fallback)

- 5.5.20: Update CLI usage documentation with find command

- 5.5.21: Committed with message: `feat: add lithos find command for interactive template selection`

## Tasks / Subtasks

- [ ] Task 1: TDD - Write failing tests for FindTemplates (AC: 5.5.10)
...
- [ ] Task 2: Implement FinderItem interface for Template (AC: 5.5.4)
...
- [ ] Task 3: TDD - Implement CLI find command (AC: 5.5.3, 5.5.10-5.5.11)
...
- [ ] Task 4: Implement non-interactive mode handling (AC: 5.5.5)
...
- [ ] Task 5: Implement output formatting (AC: 5.5.6)
...
- [ ] Task 6: Implement error and cancellation handling (AC: 5.5.7-5.5.8)
...
- [ ] Task 7: Implement logging (AC: 5.5.9)
...
- [ ] Task 8: Quality verification (AC: 5.5.12-5.5.17)
...
- [ ] Task 9: Verify requirements and update docs (AC: 5.5.18-5.5.20)
...
- [ ] Task 10: Commit changes (AC: 5.5.21)
  - [ ] Review all changes
  - [ ] Verify tests pass
