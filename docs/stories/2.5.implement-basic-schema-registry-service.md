...
- [ ] Task 1: Define SchemaRegistry interface and service (AC: 2.5.1, 2.5.6)
  ...
  - [ ] Define SchemaRegistryPort interface in `internal/ports/api/schema.go` for dependency injection
  - [ ] Implement constructor accepting SchemaLoaderPort and ConfigPort dependencies
  - [ ] Follow domain service patterns with proper encapsulation
  - [ ] Ensure service operates on domain Schema objects from SchemaLoaderPort (no JSON parsing)

- [ ] Task 2: Implement schema loading functionality (AC: 2.5.2, 2.5.5, 2.5.8)
  - [ ] Implement LoadSchemas method using SchemaLoaderPort.LoadSchemas() and LoadPropertyBank()
  ...
  - [ ] Handle schema loading errors with proper error wrapping using Result[T] pattern
  - [ ] Consume domain Schema and PropertyBank objects from SchemaLoaderPort (Story 2.4)
  - [ ] Store loaded schemas in Registry package for thread-safe access

- [ ] Task 3: Implement thread-safe schema storage (AC: 2.5.3, 2.5.4)
  - [ ] Use Registry package from `internal/shared/registry` for thread-safe schema storage
  - [ ] Implement Get(name string) Result[Schema] method for schema retrieval
  - [ ] Add proper error handling for missing schemas with SchemaNotFoundError
  - [ ] Ensure concurrent access safety for all registry operations using sync.RWMutex patterns
  - [ ] Implement List() method for schema enumeration
...
- [ ] Task 4: Implement Result pattern and error handling (AC: 2.5.7, 2.5.11)
  ...
  - [ ] Define SchemaRegistryError types for registry-specific errors
  - [ ] Handle all error conditions with appropriate error types (SchemaNotFoundError, SchemaLoadError)

- [ ] Task 5: Add comprehensive testing (AC: 2.5.9, 2.5.10)
  - [ ] Create `internal/app/schema/registry_test.go` with unit tests
  - [ ] Add tests with mocked SchemaLoaderPort and ConfigPort dependencies using test doubles
  - [ ] Test schema loading, storage, retrieval, and error conditions
  - [ ] Test concurrent access scenarios with multiple goroutines
  - [ ] Add tests for context cancellation and timeout handling
  - [ ] Test Result[T] pattern implementation and error propagation
  - [ ] Ensure test coverage meets project standards (≥85% for app layer)
  - [ ] Add integration tests with real Schema domain objects
...
- Use dependency injection for SchemaLoaderPort and ConfigPort
...
- **Integration Points**: SchemaRegistry used by SchemaValidator for validation, loads schemas via SchemaLoaderPort
...
- **Integration Points**: SchemaRegistry loads via SchemaLoaderPort, stores in Registry, accessed by SchemaValidator
...
- **Dependency Injection**: Accept SchemaLoaderPort and ConfigPort via constructor
...
- [ ] SchemaRegistryPort interface defined for dependency injection
- [ ] Service loads schemas via SchemaLoaderPort with ConfigPort integration
- [ ] Registry package used for thread-safe schema storage with concurrent access safety
- [ ] Get(name string) Result[Schema] method provides schema retrieval with proper error handling
- [ ] List() method provides schema enumeration capability
- [ ] Result[T] pattern implemented for functional error handling across all public methods
- [ ] Context cancellation supported for loading operations
- [ ] Structured error types defined (SchemaNotFoundError, SchemaLoadError)
- [ ] Domain service consumes Schema/PropertyBank domain objects without JSON parsing
- [ ] Unit tests pass with ≥85% coverage for all functionality including concurrent access
- [ ] Code follows project coding standards and naming conventions
- [ ] Proper error handling and logging implemented with structured error types
...
- Story dependencies: Requires completion of Stories 2.3 and 2.4 for domain objects and SchemaLoaderPort
- Tasks need to be completed sequentially before story can be approved
- Need clearer specification of Registry package integration patterns
- Thread-safety requirements need more detailed implementation guidance

**Critical Dependencies:**
- Story 2.3: Property domain models for PropertyBank handling
- Story 2.4: SchemaLoaderPort implementation for schema loading

**Recommendations:**
- Wait for Stories 2.3 and 2.4 completion before implementing
- Complete Task 1-5 sequentially with focus on thread-safety patterns
- Ensure Result[T] pattern is consistently applied across all public methods
