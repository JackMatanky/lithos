...
- [ ] Task 11: Enforce linting and formatting workflow after final validation
...
- [ ] Task 12: Pre-commit and commit readiness
  - [ ] Execute `pre-commit run` to confirm hooks pass
  - [ ] Stage updates and commit with a fully descriptive conventional commit message summarizing all changes
...
- Reviewed legacy implementation in `internal/shared/errors/`
- Compared Result[T] semantics with `docs/refs/aidantwoods-go-result-digest.txt`
- Execution: `GOCACHE=$(mktemp -d) go test ./...`
- Execution: `TMPDIR=$(mktemp -d) GOCACHE=$(mktemp -d) GOLANGCI_LINT_CACHE=$(mktemp -d) golangci-lint run`
- Attempted `pre-commit run --all-files` (fails: sandbox denies `sysconf` access)

### Completion Notes List

- Refactored `internal/shared/errors` to minimalist Base/Validation/Resource core with schema, frontmatter, and template facades.
- Introduced top-level `Map`/`AndThen` helpers to enable functional chaining while preserving Go generics constraints.
- Audited adapters/domain packages to adopt new error types; replaced filesystem/storage aliases with ResourceError.
- Expanded README with new usage guidance and updated architecture doc registry.
- `pre-commit run` cannot execute in sandbox due to `sysconf` permission; manual lint/test suite executed instead.

### File List

**Files Created/Modified:**
- `internal/shared/errors/result.go`
- `internal/shared/errors/types.go`
- `internal/shared/errors/frontmatter.go`
- `internal/shared/errors/schema.go`
- `internal/shared/errors/template.go`
- `internal/shared/errors/README.md`
- `internal/shared/errors/result_test.go`
- `internal/shared/errors/types_test.go`
- `internal/domain/property.go`
- `internal/app/template/executor.go`
- `internal/app/template/service.go`
- `internal/adapters/spi/template/fs_adapter.go`
- `internal/adapters/spi/schema/{loader.go,loader_test.go,parser.go,converter.go,validation.go,resolver.go,registry.go}`
- `docs/architecture/components.md`
- `docs/prd/epic-2-configuration-schema-loading.md`

**Referenced Only:**
- `docs/refs/aidantwoods-go-result-digest.txt`
- `docs/course_correction/sprint-change-proposal-schema-validator-refactoring.md`
