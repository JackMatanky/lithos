...
- `internal/adapters/spi/schema/inheritance_resolver.go` - **CREATE** - Builder pattern for inheritance resolution
- `internal/adapters/spi/schema/inheritance_resolver_test.go` - **CREATE** - Unit tests for inheritance logic

## Tasks / Subtasks

- [ ] Task 1: Implement inheritance resolution builder (AC: 2.6.1, 2.6.6)
  - [ ] Create `internal/adapters/spi/schema/inheritance_resolver.go` with builder pattern
  - [ ] Implement dependency graph building from schema Extends relationships
  - [ ] Add topological sorting for safe resolution order
  - [ ] Integrate with existing SchemaRegistryAdapter (from Story 2.5)
  - [ ] Ensure the builder consumes domain `Schema` instances supplied by SchemaRegistryAdapter without performing JSON parsing.
...
- [ ] Task 4: Integrate with SchemaRegistryAdapter (AC: 2.6.7, 2.6.8)
  - [ ] Update SchemaRegistryAdapter to call inheritance resolution after schema loading
  - [ ] Use existing Schema and Property domain models from Stories 2.2 and 2.3
  - [ ] Follow SPI adapter patterns with proper error handling
  - [ ] Ensure Result[T] pattern for inheritance resolution errors
  - [ ] Integrate inheritance resolver with SchemaRegistryAdapter loading process

- [ ] Task 5: Add comprehensive testing (AC: 2.6.9, 2.6.10)
  - [ ] Create `internal/adapters/spi/schema/inheritance_resolver_test.go` with unit tests
  - [ ] Test various inheritance scenarios: single level, multi-level, property overrides
  - [ ] Test cycle detection with different circular dependency patterns (A→B→A, A→B→C→A)
  - [ ] Test Excludes functionality for subtractive inheritance
  - [ ] Test property merging logic: parent properties → apply Excludes → merge child properties
  - [ ] Test error conditions and edge cases (invalid references, malformed inheritance)
  - [ ] Test ResolvedProperties immutability after inheritance resolution
  - [ ] Ensure test coverage meets project standards (≥95% for app layer)

- [ ] Task 6: Decompose functions for SRP compliance
  - [ ] Refactor complex resolver logic into targeted private helpers for graph creation, cycle detection, and property merging
  - [ ] Separate dependency graph building from topological sorting logic
  - [ ] Extract property merging logic into focused helper methods
  - [ ] Re-run resolver tests to confirm all acceptance criteria remain satisfied

- [ ] Task 7: Enforce linting and formatting workflow
  - [ ] Run `golangci-lint fmt`
  - [ ] Run `golangci-lint run --fix`, resolving warnings without resorting to `//nolint` unless unavoidable
  - [ ] After each fix, rerun `golangci-lint fmt` and associated tests to ensure they pass

- [ ] Task 8: Pre-commit and commit readiness
  - [ ] Execute `pre-commit` to validate repository hooks
  - [ ] Stage the changes and record a full conventional commit message summarizing inheritance-resolution updates
...
**File Locations:**
- Inheritance resolver: `internal/adapters/spi/schema/inheritance_resolver.go` (create new)
- Updated registry: `internal/adapters/spi/schema/registry.go` (enhance existing from Story 2.5)
- Tests: `internal/adapters/spi/schema/inheritance_resolver_test.go` (create new)
