# Story 3.3: Implement VaultReaderPort and FileSystemVaultReaderAdapter
...
- 3.3.2: Create `internal/adapters/spi/vault/vault_file.go` with VaultFile DTO:
...
**FileSystemVaultReaderAdapter Implementation:**

- 3.3.3: Create `internal/adapters/spi/vault/reader.go` with FileSystemVaultReaderAdapter:
  - Implements VaultReaderPort interface
  - Constructor: `NewVaultReaderAdapter(config Config, log Logger) *FileSystemVaultReaderAdapter`
...
- 3.3.10: Committed with message: `feat(vault): implement VaultReaderPort and FileSystemVaultReaderAdapter with vault scanning`
...
- [ ] Task 2: Create VaultFile DTO (AC: 3.3.2)
  - [ ] RED: Write failing test for NewVaultFile constructor
    - [ ] Write test case in `internal/adapters/spi/vault/vault_file_test.go`
    - [ ] Verify test fails with expected error message
  ...
  - [ ] RED: Write failing test for VaultFile.Content field
    ...
    - [ ] Run `go test ./internal/adapters/spi/vault` and confirm failure
  - [ ] GREEN: Create `internal/adapters/spi/vault/vault_file.go`
...
- [ ] Task 3: Implement FileSystemVaultReaderAdapter struct and constructor (AC: 3.3.3)
  ...
  - [ ] GREEN: Create `internal/adapters/spi/vault/reader.go`
    - [ ] Define FileSystemVaultReaderAdapter struct with config and log fields
    - [ ] Implement NewVaultReaderAdapter constructor
    - [ ] Add interface compliance check: `var _ VaultReaderPort = (*FileSystemVaultReaderAdapter)(nil)`
    - [ ] Run `go test ./internal/adapters/spi/vault` and verify all tests pass
    - [ ] Verify no other tests broken
  - [ ] REFACTOR:
    - [ ] Decompose into SRP components:
    ...
    - [ ] Review naming: FileSystemVaultReaderAdapter (clear), NewVaultReaderAdapter (idiomatic)
    - [ ] Add comprehensive GoDoc comments:
...
- [ ] Task 7: Run quality gates (AC: 3.3.8-3.3.9)
...
- [ ] Task 8: Commit changes (AC: 3.3.10)
  - [ ] Review all changes
  - [ ] Stage files: `git add internal/ports/spi/vault.go internal/adapters/spi/vault/`
  - [ ] Commit with message: `feat(vault): implement VaultReaderPort and FileSystemVaultReaderAdapter with vault scanning`
...
```go
type FileSystemVaultReaderAdapter struct {
    config Config
    log    zerolog.Logger
}

func (a *FileSystemVaultReaderAdapter) ScanAll(ctx context.Context) ([]VaultFile, error) {
    var files []VaultFile
    startTime := time.Now()
...
```go
func (a *FileSystemVaultReaderAdapter) ScanModified(ctx context.Context, since time.Time) ([]VaultFile, error) {
    var files []VaultFile
...
```go
func (a *FileSystemVaultReaderAdapter) Read(ctx context.Context, path string) (VaultFile, error) {
    // Validate path is within vault (prevent directory traversal)
...
- `internal/adapters/spi/vault/reader.go` - FileSystemVaultReaderAdapter
- `internal/adapters/spi/vault/vault_file.go` - VaultFile DTO
- `internal/adapters/spi/vault/file_metadata.go` - FileMetadata model (Story 1.9)
...
**Naming Standards:**
- Adapter types: FileSystem[Purpose]Adapter (FileSystemVaultReaderAdapter)
