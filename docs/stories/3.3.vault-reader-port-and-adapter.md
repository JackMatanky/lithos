...
**Vault Port Interfaces (Interface Segregation):**

- 3.3.1: Create `internal/ports/spi/vault.go` with **TWO** segregated interfaces following Interface Segregation Principle:

**VaultReaderPort Interface:**
  - Method: `Read(ctx context.Context, path string) (VaultFile, error)` - Single file read for validation
  - Purpose: FileSpec validation and single file access
  - Used by: FrontmatterService for FileSpec validation
  - Security: Path validation prevents directory traversal

**VaultScannerPort Interface:**
  ...
  - Purpose: Bulk vault scanning for indexing operations
  - Used by: VaultIndexer for building search indices
  - Performance: ScanModified enables incremental indexing optimization

- GoDoc: Reference business-level semantics, FR9 error handling, Interface Segregation Principle, and explain VaultFile DTO return type
...
- 3.3.3: Create `internal/adapters/spi/vault/reader.go` with VaultReaderAdapter:
  - Implements **BOTH** VaultReaderPort and VaultScannerPort interfaces (single adapter, dual interface compliance)
  ...
  - Interface compliance: `var _ VaultReaderPort = (*VaultReaderAdapter)(nil)` and `var _ VaultScannerPort = (*VaultReaderAdapter)(nil)`
  - Ignores cache directories (`.lithos/`) during scanning operations
...
- [x] Task 5: Implement ScanModified method (AC: 3.3.5)
...
| 2025-10-31 | 1.7     | Updated story to reflect Interface Segregation: VaultReaderPort split into VaultReaderPort (single file) and VaultScannerPort (bulk scanning). Adapter implements both interfaces. | Sarah (PO)         |
