...
- 4.3.1: Implement `CommandOrchestrator.NewNote(ctx context.Context, templateID TemplateID) (Note, error)` method following the documented ten-step workflow
- 4.3.2: Document workflow steps in method comments referencing `docs/architecture/components.md#commandorchestrator`
- 4.3.3: Verify each step executes in correct order with proper error handling

**Workflow Steps:**

- 4.3.4: Step 1 - Load template via `TemplateEngine.Load(ctx, templateID)`
- 4.3.5: Step 2 - Render template via `TemplateEngine.Render(ctx, template)`
- 4.3.6: Step 3 - Extract frontmatter via `FrontmatterService.Extract([]byte(rendered))`
- 4.3.7: Step 4 - Validate frontmatter via `FrontmatterService.Validate(ctx, frontmatter)`
- 4.3.8: Step 5 - Generate NoteID from frontmatter (filename → title slug → UUID)
- 4.3.9: Step 6 - Resolve file path (template context → frontmatter → default)
- 4.3.10: Step 7 - Create Note object with ID, Content, Frontmatter
- 4.3.11: Step 8 - Persist to vault via `VaultWriterPort.Persist(ctx, note, path)`
- 4.3.12: Step 9 - Persist to cache via `CacheWriterPort.Persist(ctx, note)` (log warning on failure, don't abort)
- 4.3.13: Step 10 - Return Note object

**Testing:**

- 4.3.14: Create unit tests in `internal/app/orchestrator/orchestrator_test.go`:
...
- 4.3.15: All tests pass: `go test ./internal/app/orchestrator`

- 4.3.16: All linting passes: `golangci-lint run --fix internal/app/orchestrator`

- 4.3.17: Committed with message: `feat(orchestrator): implement schema-driven NewNote workflow with dual-write pattern`

## Tasks / Subtasks

- [ ] Task 1: Implement NewNote method skeleton (AC: 4.3.1-4.3.3)
...
- [ ] Task 2: Implement steps 1-4 (Load, Render, Extract, Validate) (AC: 4.3.4-4.3.7)
...
- [ ] Task 3: Implement step 5 (Generate NoteID) (AC: 4.3.8)
...
- [ ] Task 4: Implement step 6 (Resolve File Path) (AC: 4.3.9)
...
- [ ] Task 5: Implement steps 7-10 (Create Note, Persist, Return) (AC: 4.3.10-4.3.13)
...
- [ ] Task 6: Run quality gates (AC: 4.3.15-4.3.16)
...
- [ ] Task 7: Commit changes (AC: 4.3.17)
  - [ ] Review all changes
...
4. **Generate NoteID:** Derive NoteID from frontmatter fields (filename, title slug, or UUID)
...
4. **DO return success if vault write succeeds** - Cache can be rebuilt
