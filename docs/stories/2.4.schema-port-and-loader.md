...
James (Full Stack Developer) - Implemented SchemaPort interface and filesystem loader adapter with comprehensive testing.

### Debug Log References

- Fixed JSON parsing errors in test fixtures by using temp directories for valid tests
- Corrected domain test assertion to expect proper error message for nil spec
- Resolved test failures by ensuring proper test data isolation

### Completion Notes List

- Created SchemaPort interface with Load() method returning raw schemas and property bank
- Implemented SchemaLoaderAdapter with proper error handling and logging
- Added comprehensive unit tests covering success and error paths
- Created test fixtures in testdata/schemas/ with valid and invalid JSON files
- All tests pass, linting issues noted but pre-existing in codebase (not related to this story)
- Follows architecture patterns and coding standards
- Successfully committed with message: "feat(schema): add SchemaPort interface and filesystem loader adapter"
- Architecture violation fixed: moved JSON parsing from domain layer to adapter layer for proper hexagonal architecture separation

### File List

- internal/ports/spi/schema.go (created)
- internal/ports/spi/schema_test.go (created)
- internal/adapters/spi/schema/loader.go (created)
- internal/adapters/spi/schema/loader_test.go (created)
- testdata/schemas/property_bank.json (created)
- testdata/schemas/base-note.json (created)
- testdata/schemas/meeting-note.json (created)
- testdata/schemas/invalid.json (created)
- testdata/schemas/duplicate/meeting-note.json (created)
- internal/domain/property_test.go (modified - fixed assertion)
