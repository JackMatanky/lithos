...
| 2025-10-31 | 2.0     | DDD refactoring: Property entity with ID, removed PropertyRef/IProperty  | James (Dev)        |
...
- 2025-10-31 DDD Refactoring Phase 1:
  - go test ./internal/domain -v: All 42 tests passing including new entity tests
  - TDD approach: RED → GREEN → REFACTOR cycles completed successfully
  - Property entity with hash-based ID generation implemented
  - PropertyRef and IProperty interface removed from domain layer
  - Schema model updated to use []Property instead of []IProperty
  - Minor golines formatting issues remaining (non-blocking)
...
- **DDD Entity Transformation**: Transformed Property to DDD entity with hash-based ID generation using SHA256
- **PropertyBank Membership**: Added Property.InPropertyBank() method for membership checking without PropertyRef
- **Domain Layer Cleanup**: Removed PropertyRef model and IProperty interface from domain layer as planned
- **Entity Constructor**: Implemented NewProperty constructor with auto-generated deterministic IDs
- **Schema Integration**: Updated Schema model to use []Property instead of []IProperty interface
- **Test Updates**: Enhanced existing tests and added new tests for entity semantics with ID-based operations

### File List

- internal/domain/property.go: Property DDD entity with ID and validation logic (110 lines, updated)
- internal/domain/property_spec.go: PropertySpec interface and all variants (179 lines, unchanged)
- internal/domain/property_test.go: Property unit tests including entity tests (200 lines, updated)
- internal/domain/property_spec_test.go: PropertySpec unit tests (237 lines, unchanged)
- internal/domain/schema.go: Schema aggregate using Property entities (215 lines, updated)
- internal/domain/schema_test.go: Schema tests updated for Property entities (390 lines, updated)
