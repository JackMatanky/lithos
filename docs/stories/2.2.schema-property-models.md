...
func (p Property) Validate(ctx context.Context) error {
    ...
    if p.Spec == nil {
        return &ValidationError{
            Message: "property spec cannot be nil",
        }
    }

    // Delegate to PropertySpec
    if err := p.Spec.Validate(ctx); err != nil {
...
func (f FileSpec) Validate(ctx context.Context) error {
    ...
    if f.Directory != "" {
        pattern := strings.TrimPrefix(f.Directory, "^") // Remove negation
        if _, err := regexp.Compile(pattern); err != nil {
