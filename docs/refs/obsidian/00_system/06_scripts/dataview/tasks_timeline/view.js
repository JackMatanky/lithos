...
function getMeta(tasks) {
  for (i = 0; i < tasks.length; i++) {
    ...
    // Date Created Syntax
    var createdMatch = taskText.match(
      /➕ *(\d{4}-\d{2}-\d{2})/
    );
    if (createdMatch) {
    ...
    // Task Type Syntax
    var typeMatch = taskText.match(
      /((_action_item)|(_meeting)|(_phone_call)|(_interview)|(_appointment)|(_event)|(_gathering)|(_hangout)|(_habit)|(_morning_ritual)|(_workday_startup_ritual)|(_workday_shutdown_ritual)|(_evening_ritual))/
    );
    if (typeMatch) {
      tasks[i].text = tasks[i].text.replace(typeMatch[0], "");
    }
