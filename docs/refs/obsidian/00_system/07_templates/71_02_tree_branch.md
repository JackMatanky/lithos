...
const pkm_obj_arr = await tp.user.file_name_alias_by_class_type({
  dir: pkm_dir,
  file_class: "pkm_tree",
  type: "category",
});
...
const category_value_link = yaml_li(`[[${pkm_value}|${pkm_name}]]`);

const branch_value_link =
  (field_value_link =
  subject_value_link =
  topic_value_link =
  subtopic_value_link =
...
const pkm_info_field = await pkm_info("Fields");
...
const tree_hierarchy = ["Category"].map(
  (x) =>
...
const tree_callout = [tree_call_title, call_start, tree_call_category].join(
  new_line
);
