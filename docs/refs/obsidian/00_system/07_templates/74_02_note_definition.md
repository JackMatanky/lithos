...
/*                       SET DESCRIPTION                      */
/* ---------------------------------------------------------- */
const about_define = await tp.system.prompt(
  `Define the term?${two_new_line}Term:${space}${title}`,
  null,
  false,
  true
);
const about_explain = await tp.system.prompt(
  `Explain the ${type_value} in simple terms?${two_new_line}Term:${space}${title}${new_line}Definition:${space}${about_define}?`,
  null,
  false,
  true
);
const about = `**Definition**:${space}${about_define}${two_new_line}**Explanation**:${space}${about_explain}`;
