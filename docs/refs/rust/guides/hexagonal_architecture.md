...
2. [Part I: Anatomy of a bad Rust application](#anatomy-of-a-bad-rust-application)
	1. [What problems does hexagonal architecture solve?](#what-problems-does-hexagonal-architecture-solve)
		2. [Hard dependencies and hexagonal architecture: how to make the right call](#hard-dependencies-and-hexagonal-architecture-how-to-make-the-right-call)
3. [Part II: Separation of concerns, the Rust way](#separation-of-concerns-the-rust-way)
	1. [Getting started with hexagonal architecture](#getting-started-with-hexagonal-architecture)
		2. [The repository pattern in Rust](#the-repository-pattern-in-rust)
		3. [Domain models](#domain-models)
		4. [Error types and hexagonal architecture](#error-types-and-hexagonal-architecture)
		5. [Implementing `AuthorRepository`](#implementing-authorrepository)
		6. [Everything but the kitchen async](#everything-but-the-kitchen-async)
		7. [From the Very Bad Application to the merely Bad Application](#from-the-very-bad-application-to-the-merely-bad-application)
		8. [Testing HTTP handlers with injected repositories](#testing-http-handlers-with-injected-repositories)
4. [Part III: `Service`, the heart of hexagonal architecture](#service-the-heart-of-hexagonal-architecture)
	1. [Introducing the `Service` trait](#introducing-the-service-trait)
		2. [Why hexagons?](#why-hexagons)
		3. [How to choose the right domain boundaries](#how-to-choose-the-right-domain-boundaries)
		4. [A Rust project template for hexagonal architecture](#a-rust-project-template-for-hexagonal-architecture)
		5. [Make an informed decision](#make-an-informed-decision)
5. [Part IV: Trade-offs of hexagonal architecture in Rust](#trade-offs-of-hexagonal-architecture-in-rust)
6. [Part V: Advanced hexagonal architecture in Rust](#advanced-hexagonal-architecture-in-rust)
...
Now, this is a big topic. Huge (O'Reilly, hit me up). I'm going to publish it section by section, releasing the next only once you've had a chance to digest the last and tackle the [exercises](#exercises) for each new concept. Bookmark this page if you don't want to miss anything – I'll add every new section here.
...
The Very Bad Application is the most common way to write production services. Your company will have code that looks just like it. writes its tutorial app in a similar way. In fairness, it has its hands full with teaching us Rust, and it only promised to *get* us to production, not keep us there.
...
Firstly, why is `main` configuring HTTP middleware [==1==](#coderef-1)? In fact, it looks like `main` needs an intimate understanding of the whole axum crate just to get the server running [==4==](#coderef-4)! axum isn't even part of our application – it's a third-party dependency that has escaped containment.
...
Look, there's that hard dependency on sqlx [==5==](#coderef-5), polluting the system on cue 🙄. And holy good god, our HTTP handler is orchestrating database transactions [==7==](#coderef-7). An HTTP handler shouldn't even know what a database *is*, but this one knows SQL!
...
And the horrifying consequence of this is that the handler also has to understand the specific error type of the database crate – and the database itself [8](#coderef-8):
...
And don't come at me with "it's fine, we can still integration test it", because that's not enough. Look at how complex the error handling is. We've got inline request body validation [==6==](#coderef-6), transaction management [==7==](#coderef-7), and sqlx errors [==8==](#coderef-8) in one function.
...
> [!question] Really? All of my projects?
>
...
> [!info] The Single Responsibility Principle
>
...
inform de that understands the HTTP request-response cycle shouldn't also understand SQL. Code that needs a database doesn't need to know how that database is implemented. These could not be more different concerns.
...
`create_author` takes a reference to the data required to create an author [==9==](#coderef-9), and returns a `Result` containing either a saved `Author`, or a specific error type describing everything that might go wrong while creating an author [==10==](#coderef-10). Right now, that's just the existence of duplicate authors, but we'll come back to error handling.
...
Now, these aren't very exciting models (they'll get more exciting when we talk about identifying the correct domain boundaries and the special concern of authentication in [Part III](#service-the-heart-of-hexagonal-architecture)). But they demonstrate how the domain defines what data flowing through the system must look like.
...
Why do we distinguish `CreateAuthorRequest`[12](#coderef-12)from `Author`[==11==](#coderef-11)? Surely we could represent both saved and unsaved `Author`s as
...
Let's zoom in on `CreateAuthorError`[13](#coderef-13). It reveals some important properties of domain models and traits.
...
> [!info] Exhaustive vs. non-exhaustive enums
>
...
The domain doesn't know anything about database implementations. It doesn't know about transactions. It doesn't know about the fire hazards posed by large data centers and your pyromaniac intern. It's oblivious to retry strategies, cache layers and dead letter queues (we'll talk about these in [Part V: *Advanced Hexagonal Architecture in Rust*](#advanced-hexagonal-architecture-in-rust)).
...
Wrapping types like `sqlx::SqlitePool` has the benefit of encapsulating a third-party dependencies within code of our own design. Remember the Very Bad Application's leaky `main` function[1](#coderef-1)? Wrapping external libraries and exposing only the functionality your application needs is how we plug the leaks.
...
At the point where most applications are instantiating databases, the only reasonable thing to do with an error is log it to `stdout` or some log aggregation service. `Sqlite::new` simply wraps any sqlx error it encounters with some extra context[15](#coderef-15).

> [!tip] Add context to opaque errors
>
...
`Sqlite` 's implementation of `AuthorRepository` knows all about SQLite error codes, and transforms any error corresponding to a duplicate author into the domain's preferred representation[18](#coderef-18).

Of course, `Sqlite`, not being part of the domain's Garden of Eden, may encounter an error that the domain can't do much with[19](#coderef-19).
...
What about retry handling? Good question. We'll cover that in [Part V: *Advanced Hexagonal Architecture in Rust*](#advanced-hexagonal-architecture-in-rust).
...
Since our `Author` and `CreateAuthorError` are both `Send`, a `Future` that wraps them can be too[20](#coderef-20).
...
Ok, the walkthrough. `create_author` has access to an `AuthorRepository`[22](#coderef-22), which it makes good use of. But first, it converts the raw `CreateAuthorHttpRequestBody` it received from the client into the holy domain representation[==23==](#coderef-23). Here's how:
...
Thanks to the pains we took to define all the errors an `AuthorRepository` is allowed to return, constructing an HTTP response is dreamy. In the error case, we map seamlessly to a serializable `ApiError` using `ApiError::from`[24](#coderef-24):
...
If the author was found to be a duplicate, it means the client's request was correctly structured, but that the contents were unprocessable. Hence, we're aiming to respond `422 Unprocessable Entity`[27](#coderef-27).

Important detail alert! Do you see how we're manually building an error message at[28](#coderef-28), even though `CreateAuthorError::to_string` could have produced this error for us?
...
If we get an error the domain didn't expect – `CreateAuthorError::Unknown` here – that maps straight to `InternalServerError`[26](#coderef-26).
...
> [!info] Errors into HTTP responses
>
...
Finally, our success case[25](#coderef-25). We take a reference to the returned `Author` and transform it into its public API counterpart. It gets sent on its way with status `201 Created`.
...
`MockAuthorRepository` is defined to hold the `Result` it should return in response `AuthorRepository::create_author` calls[29](#coderef-29) [==30==](#coderef-30).

The rather nasty type signature at[29](#coderef-29) is due to the fact that `AuthorRepository` has a `Clone` bound, which means `MockAuthorRespository` must be `Clone`.
...
At[31](#coderef-31), we construct a `MockAuthorRepository` with an arbitrary success `Result`. We expect that a `Result::Ok(Author)` from the repo should produce a `Result::Ok(ApiSuccess<CreateAuthorResponseData>)` from the handler[==32==](#coderef-32).

This situation is simple to set up – we just call the `create_author` handler with a `State` object constructed from the `MockAuthorRepository` in place of a real one[33](#coderef-33). The assertions are self-explanatory.
...
> [!note] Is this a plausible scenario?
>
...
>
> In fact, I got the data model for the _How To Code It_ CMS wrong on my first attempt, and had to rebuild it 🙃.
>
...
>
> How often these occur is a function of both scale and growth rate. We'll tackle these topics in depth in [Part IV](https://www.howtocodeit.com/guides/master-hexagonal-architecture-in-rust#trade-offs-of-hexagonal-architecture-in-rust).
...
`AuthorMetrics`[34](#coderef-34) describes an aggregator of author-related metrics, such as a time-series database. `AuthorNotifier`[==35==](#coderef-35)triggers notifications to authors.

> [!warning] Finding the right abstraction
>
...
The `Service` struct encapsulates the dependencies required to execute our business logic[36](#coderef-36).

The implementation of `AuthorService::create_author`[37](#coderef-37) illustrates why we don't want to embed these calls directly in handler code, which has enough work to do just managing the request-response cycle.
...
To do this, `main` needs to know which adapters to slot into the domain's ports[38](#coderef-38). This example uses an SQLite `AuthorRepository`, Prometheus `AuthorMetrics` and an email-based `AuthorNotifier`.

It combines these implementations of the domain traits into an `AuthorService`[39](#coderef-39) using the author domain's `Service` constructor.

Finishing up, it injects the `AuthorService` into an HTTP server and runs it[40](#coderef-40).
...
![[how_to_code_it_master_hexagonal_architecture_in_rust_ports_adapters_diagram.webp]]
...
> [!info] Further reading on domain-driven design
>
...
>
> An `Author` for a real blogging platform might be a complex aggregation of user data, posts and followers. The domain represents this as a single compound type ("aggregate" is the domain-driven design jargon), but an SQL database will store this data across many tables.
>
...
Compared to the Very Bad Application[1](#coderef-1)

, our hexagonal app takes a lot more code to achieve the same result. Instead of a raw, axum HTTP handler that simply takes the request body and shoves it straight into an SQLite database, we have multiple layers of abstraction:
...
> [!warning] "Founders don't have time for tests"
>
