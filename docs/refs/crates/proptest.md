...
publisher:
  - "[[github.io]]"
url: "https://proptest-rs.github.io/proptest/print.html"
about: "Usage documentation for the proptest and proptest-derive crates"
status: "undetermined"
type: "webpage"
file_class: "lib_webpage"
cssclasses:
date_created: 2026-05-14T14:08
date_modified: 2026-05-14T14:08
tags:
  - "clippings"
---
# Proptest
...
See the [changelog](https://github.com/proptest-rs/proptest/blob/master/proptest/CHANGELOG.md) for a full list of substantial historical changes, breaking and otherwise.
...
This part of the book is dedicated to introductory material, such as tutorials, and general usage suggestions. It does *not* contain reference documentation; for that, please see the [rustdoc documentation](https://docs.rs/proptest/latest/proptest/).
...
If we look at the top directory after the test fails, we’ll see a new `proptest-regressions` directory, which contains some files corresponding to source files containing failing test cases. These are [*failure persistence*](https://proptest-rs.github.io/proptest/proptest/failure-persistence.html) files. The first thing we should do is add these to source control.
...
The `proptest!` macro has some additional syntax, including for setting configuration for things like the number of test cases to generate. See its [documentation](https://docs.rs/proptest/latest/proptest/macro.proptest.html) for more details.
...
Please make sure to read the [introduction to this tutorial](https://proptest-rs.github.io/proptest/proptest/tutorial/index.html) before starting this section.

The [*Strategy*](https://docs.rs/proptest/latest/proptest/strategy/trait.Strategy.html) is the most fundamental concept in proptest. A strategy defines two things:
...
Other compound strategies include fixed-sizes arrays of strategies and `Vec` s of strategies (which produce arrays or `Vec` s of values parallel to the strategy collection), as well as the various strategies provided in the [collection](https://docs.rs/proptest/latest/proptest/collection/index.html) module.

## Syntax Sugar: proptest!

Now that we know about compound strategies, we can understand how the [`proptest!`](https://docs.rs/proptest/latest/proptest/macro.proptest.html) macro works. Our example from the prior section can be rewritten using that macro like so:
...
To simplify this task, proptest includes the [`prop_compose!`](https://docs.rs/proptest/latest/proptest/macro.prop_compose.html) macro. Before going into details, here’s our code from above rewritten to use it.
...
The syntax sugar for defining strategies for `enum` s is currently somewhat limited. Creating such strategies with `prop_compose!` is possible but generally is not very readable, so in most cases defining the function by hand is preferable.
...
For particularly complex enum cases, it can be helpful to extract the strategy for that case to a separate strategy. Here, [`prop_compose!`](https://docs.rs/proptest/latest/proptest/macro.prop_compose.html) can be of use.
...
Global filtering results when a test itself returns `Err(TestCaseError::Reject)`. The [`prop_assume!`](https://docs.rs/proptest/latest/proptest/macro.prop_assume.html) macro provides an easy way to do this.
...
In some cases, where it makes sense to define a canonical strategy, such as in the [JSON AST example](https://proptest-rs.github.io/proptest/proptest/tutorial/recursive.html), it is a good idea to implement `Arbitrary`.

The experimental [`proptest-derive` crate](https://proptest-rs.github.io/proptest/proptest-derive/index.html) can be used to automate implementing `Arbitrary` in common cases.
...
For the API reference documentation, please see the [rustdoc documentation for the `proptest` crate](https://docs.rs/proptest/latest/proptest/).
...
There is some initial investment needed to set the test up and it usually takes a bit more time to run than simple prop tests, but if correctness is important for your use case, you’ll be rewarded with a test that is so effective at discovering bugs it might feel almost magical, but as you’ll see, [you could have easily implemented it yourself](https://proptest-rs.github.io/proptest/proptest/state-machine.html#how-does-it-work). Also, once you have the test setup, it is much easier to extend it and add new properties to check.
...
The state machine testing support for Proptest is heavily inspired by the Erlang’s eqc\_statem (see the paper [Finding Race Conditions in Erlang with QuickCheck and PULSE](https://smallbone.se/papers/finding-race-conditions.pdf)) with some key differences. Most notably:

- Currently, only sequential strategy is supported, but a concurrent strategy is planned to be added at later point.
- There are no “symbolic” variables like in eqc\_statem. The state for the abstract (reference) state machine is separate from the state of the system under test.
...
It is recommended to have a basic working understanding of the [`proptest` crate](https://proptest-rs.github.io/proptest/proptest-derive//proptest/index.html) before getting into this part of the documentation.
...
In order to use `proptest-derive` on a type *not* in a test module without also depending on proptest for your main build, you must currently manually gate off the related annotations. This is something we plan to [improve in the future](https://github.com/proptest-rs/proptest/pull/106).
...
The `filter` modifier allows filtering values generated for a field via rejection sampling. Since rejection sampling is inefficient and interferes with shrinking, it should only be used for conditions that are very rare or are unfeasible to express otherwise. In many cases, [`strategy`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#strategy) can be used to more directly express the desired behaviour without rejection sampling. See the documentation for [`prop_filter`](https://docs.rs/proptest/latest/proptest/strategy/trait.Strategy.html#method.prop_filter) for more details.

The argument to the modifier must be a valid argument for the second parameter of [`prop_filter`](https://docs.rs/proptest/latest/proptest/strategy/trait.Strategy.html#method.prop_filter).
...
- The item referencing the type parameter does not have any proptest modifiers which replace the usual use of `Arbitrary`, such as [`skip`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#skip) or [`value`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#value).
...
See the [`param` modifier](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#param) for more information on how parameters work.
...
The [`Arbitrary` trait](https://docs.rs/proptest/latest/proptest/arbitrary/trait.Arbitrary.html) specifies a `Parameters` type which is used to control generation. By default, the `Parameters` type is a tuple of the parameters which are automatically passed to other `Arbitrary` implementations.
...
If applied to an enum variant or field, `params` specifies the parameters type for just that item, as if its type had an `Arbitrary` implementation taking that type. In this case, either [`value`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#value) or [`strategy`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#strategy) *must* be specified since the parameter type will not generally be compatible with the normal `Arbitrary` invocation (and in cases where it is, `params` would be useless if not used).

Any expressions (such as in the [`value`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#value) and [`strategy`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#strategy) modifiers) underneath an item with the `params` modifier has access to a variable named `params` which is of the type passed in `#[proptest(params = ..)]`.
...
The `regex` modifier is equivalent to using the [`strategy`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#strategy) modifier and enclosing the string in [`string_regex`](https://docs.rs/proptest/latest/proptest/string/fn.string_regex.html) or [`bytes_regex`](https://docs.rs/proptest/latest/proptest/string/fn.bytes_regex.html). It can only be applied to fields of type `String` or `Vec<u8>`.
...
Using `value` is equivalent to using [`strategy`](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html#strategy) and enclosing the value in `LazyJust`.
...
This error occurs when `#[derive(Arbitrary)]` is used on a type which has any [lifetime parameters](https://doc.rust-lang.org/stable/book/second-edition/ch10-03-lifetime-syntax.html#lifetime-annotations-in-struct-definitions). For example:
...
2. Second, we cannot mechanically tell which variant out of `signed` and `unsigned` to generate. While we could allow you to tell the macro, with an attribute such as `#[proptest(select)]` on the variant, we have opted for a more conservative approach for the time being. If you have a use case for `#[derive(Arbitrary)]` on `union` types, please reach out on the [issue tracker](https://github.com/proptest-rs/proptest).

## E0003

This error occurs when `#[derive(Arbitrary)]` is used on a struct which contains known [uninhabited types](https://doc.rust-lang.org/nomicon/exotic-sizes.html#empty-types). This in turn means the struct itself is uninhabited and so it there is no sensible `Arbitrary` implementation since values of the struct cannot be produced.
...
This error occurs if `#[derive(Arbitrary)]` is used on an enum whose variants are all uninhabited, using the same logic as described for [`E0003`](https://proptest-rs.github.io/proptest/proptest-derive/errors.html#e0003). As a result, the enum itself is totally uninhabited.
...
Please see the [modifiers reference](https://proptest-rs.github.io/proptest/proptest-derive/modifiers.html) to see what modifiers are available.
...
- Pass a malformed type in the string, as in the example above. (See also [caveat on syntax](https://proptest-rs.github.io/proptest/proptest-derive/errors.html#valid-rust-syntax).)
...
- Pass a string expression which is not valid Rust syntax, as in the above example. (See also [caveat on syntax](https://proptest-rs.github.io/proptest/proptest-derive/errors.html#valid-rust-syntax).)
