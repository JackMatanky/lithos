...
_Update 2025-10-31: Documented map-based property merging and single-pass hydration; existing coverage remains valid._
...
func TestSchemaLoaderAdapter_Load_Success(t *testing.T) {
    ...
    assert.ElementsMatch(t, []string{"base-note", "meeting_note"}, pluckNames(schemas))
...
func TestSchemaLoaderAdapter_Load_DuplicateNames(t *testing.T) {
    ...
    assert.Contains(t, schemaErr.Message, "meeting_note")
...
duplicate/
  ...
  daily-note.json
  meeting_note.json
  nested/meeting_note.json (duplicate name)
unknown-fields/
  property_bank.json (includes extra keys)
  base_note.json (includes custom metadata)
```
