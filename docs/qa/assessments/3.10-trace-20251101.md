...
#### AC4: QueryService PathQuery and BasenameQuery methods must work correctly with the new NoteID scheme

**Coverage: FULL**

Given-When-Then Mappings:

- **Unit Test**: `internal/app/query/service_test.go::TestQueryService_PathQuery_ExistingPath`
  - Given: QueryService with populated indices
  - When: PathQuery is called with a valid path
  - Then: Correct note is returned

- **Unit Test**: `internal/app/query/service_test.go::TestQueryService_BasenameQuery_ExistingBasename`
  - Given: QueryService with notes having path-based NoteIDs
  - When: BasenameQuery is called with a basename
  - Then: All notes with that basename are returned

- **Unit Test**: `internal/app/query/service_test.go::TestQueryService_BasenameQuery_MultipleMatches`
  - Given: Multiple notes with same basename in different paths
  - When: BasenameQuery is called
  - Then: All matching notes are returned

- **Integration Test**: `tests/integration/duplicate_basename_test.go::TestDuplicateBasenameHandling_Integration`
  - Given: Indexed vault with duplicate basenames
  - When: BasenameQuery is called for "meeting"
...
- **Integration Test**: `tests/integration/duplicate_basename_test.go::TestDuplicateBasenameHandling_Integration`
  ...
  - Then: Indices are populated allowing BasenameQuery to return multiple notes with same basename
...
- **Unit Test**: `internal/app/query/service_test.go::TestQueryService_IDQuery_ExistingNote`
  - Given: QueryService with populated indices
  - When: IDQuery is called with existing NoteID
  - Then: Correct note is returned

- **Unit Test**: `internal/app/query/service_test.go::TestQueryService_FileClassQuery_ExistingClass`
  - Given: QueryService with notes of different file classes
  - When: FileClassQuery is called
