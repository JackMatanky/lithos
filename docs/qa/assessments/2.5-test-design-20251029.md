...
func TestSchemaRegistryAdapter_RegisterAndLookup_Success(t *testing.T) {
    ...
    schemas := []Schema{{Name: "meeting_note"}}
    ...
    schema, err := registry.GetSchema(context.Background(), "meeting_note")
    require.NoError(t, err)
    assert.Equal(t, "meeting_note", schema.Name)
    ...
    schema2, _ := registry.GetSchema(context.Background(), "meeting_note")
    prop2, _ := registry.GetProperty(context.Background(), "standard_title")
    assert.Equal(t, "meeting_note", schema2.Name)
    assert.Equal(t, "title", prop2.Name)

    assert.True(t, registry.HasSchema(context.Background(), "meeting_note"))
