...
- [x] `get_raw_schema_view(id)` added to `ReadRepository`
- [x] `find_raw_schema_view_by_path(path)` added to `ReadRepository`
- [x] `save_raw_schema_view(id, view)` added to `WriteRepository`
...
- Scope is actionable and constrained to `ReadRepository`, `WriteRepository`, and `storage_v2` adapters.
...
1. `ReadRepository::get_raw_schema_view(id: SchemaId) -> Result<Option<RawSchemaView>, SchemaStorageV2Error>`
2. `ReadRepository::find_raw_schema_view_by_path(path: &RelativePath) -> Result<Option<RawSchemaView>, SchemaStorageV2Error>`
3. `WriteRepository::save_raw_schema_view(id: SchemaId, view: &RawSchemaView) -> Result<(), SchemaStorageV2Error>`
