...
**REOPENED (2026-05-12)**: The unified `Repository` trait is being split into `ReadRepository` and `WriteRepository`. The implementation must also be split into `read.rs` and `write.rs` files within `schema/storage_v2/`.
...
### 1. Seam Architecture
- **Trait**: `Repository` defined in `schema/repository.rs`.
...
- **Naming**:
  ...
  - Trait: `Repository` → rename to `Repository` after legacy migration
...
- `schema/storage_v2/read.rs`: Contains `impl Repository for SchemaRedbRepository` block with `find_schema_by_id` method.
- `schema/storage_v2/write.rs`: Contains `impl Repository for SchemaRedbRepository` block with `save_schema` method.
...
- Write test using `Repository::save_schema` and `find_schema_by_id` — fails because trait doesn't exist.
...
- Create `write.rs` with `impl Repository for SchemaRedbRepository` block containing `save_schema` method — uses `store.write(|tx| ...)` with inline rkyv serialization and table insert.

**Phase 3: Read Path**
- Create `read.rs` with `impl Repository for SchemaRedbRepository` block containing `find_schema_by_id` method — uses `store.read(|tx| ...)` with inline table get and rkyv deserialize.
...
- [ ] `ReadRepository` and `WriteRepository` traits exist in `schema/repository.rs`.
- [ ] `Repository` trait extends both Read and Write variants.
- [ ] `schema/storage_v2/read.rs` exists and implements `ReadRepository`.
- [ ] `schema/storage_v2/write.rs` exists and implements `WriteRepository`.
...
- [x] `trait Repository` exists in `schema/repository.rs` with `save_schema` and `find_schema_by_id`.
...
- [x] `impl Repository for SchemaRedbRepository` in `schema/storage_v2/core.rs` with `save_schema` and `find_schema_by_id`.
...
**Category:** enhancement
**Summary:** Split Repository into segregated Read/Write traits and implementation files.

**Current behavior:**
The `Repository` trait is a single unified interface. The implementation `SchemaRedbRepository` is currently in `storage_v2/core.rs`.

**Desired behavior:**
1. `ReadRepository` contains `find_schema_by_id`.
2. `WriteRepository` contains `save_schema`.
3. `Repository` is an empty marker trait extending both.
...
**Key interfaces:**
- `ReadRepository` / `WriteRepository` — the new segregated contracts.
- `SchemaRedbRepository` — the implementation struct whose impl blocks will be moved.

**Acceptance criteria:**
- [x] `ReadRepository` and `WriteRepository` traits exist in `schema/repository.rs`.
- [x] `Repository` trait extends both Read and Write variants.
- [x] `schema/storage_v2/read.rs` exists and implements `ReadRepository`.
- [x] `schema/storage_v2/write.rs` exists and implements `WriteRepository`.
...
- `ReadRepository` with 3 methods: `find_schema_by_id`, `find_many_schemas_by_id`, `find_raw_schema_views_by_paths`
- `WriteRepository` with 2 methods: `save_schema`, `save_many_schemas`
- `Repository` as empty marker trait with blanket impl for any type implementing both Read and Write

**Implementation Split** (`schema/storage_v2/`):
```
schema/storage_v2/
├── mod.rs      # Struct with pub(crate) store field + blanket impl (39 lines)
├── read.rs     # impl ReadRepository + comprehensive tests (443 lines)
├── write.rs    # impl WriteRepository + comprehensive tests (192 lines)
