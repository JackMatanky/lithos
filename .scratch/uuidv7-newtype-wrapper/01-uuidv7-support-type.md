...
## Triage Notes (2026-05-09)

> *This was generated by AI during triage.*

**What was established:**
- UUID crate docs explicitly deprecate `parse_str` in favor of `try_parse`
- Validation logic lives in `try_from_uuid()` when it should live in `TryFrom<Uuid>` impl
- `try_from_uuid()` is not `const fn` and provides no benefit over `TryFrom`
- 15 call sites across 8 files use `UuidV7::try_from_uuid()` — all need migration to `Uuid::try_from()`

**What needs to happen:**
- [x] Remove `try_from_uuid()` method entirely
- [x] Migrate all callers to `Uuid::try_from(uuid)`
- [x] Move validation logic into `TryFrom<Uuid>` impl
- [x] Replace `parse_str` → `try_parse` in `UuidV7::parse()`
- [x] Update domain ID wrappers (TemplateId, SchemaId, PropertyId, NoteId)
- [x] Update tests and benchmarks

**State: needs-rework → completed (2026-05-09)**

## Verification

- All 985 unit tests pass
- All 150 doc tests pass
- No remaining references to `try_from_uuid` or `parse_str` in uuid.rs
- All 15 call sites migrated from `UuidV7::try_from_uuid(x)` to `UuidV7::try_from(x)`

## Second Review (2026-05-09) — UUID Crate Best Practice Audit

### Issues Found and Fixed

1. **Test `parse_rejects_invalid_string` asserted only `unwrap_err()`** — Didn't verify the error was the `Parse` variant. If `try_parse` changed behavior and returned the inner UUID, the test would silently pass for the wrong reason. **Fix**: Assert `matches!(err, UuidV7Error::Parse(_))`.

2. **Stale test name `try_from_uuid_rejects_non_v7`** — Referenced the removed `try_from_uuid()` method. **Fix**: Renamed to `try_from_rejects_non_v7`.

### NonIssues (Intentionally Not Changed)

- **Cross-type `PartialEq<Uuid>` / `PartialOrd<Uuid>`**: The crate's `NonNilUuid` provides these for ergonomic comparison without `.as_uuid()`. `UuidV7` intentionally omits them — the v7 constraint is stronger than non-nil. Requiring explicit `.as_uuid()` or `.into_uuid()` conversion prevents accidental mixing of validated (v7) and unvalidated (any version) UUIDs.

- **`const fn` construction path (`NonNilUuid::new()` → `Option<Self>`)**: v7 UUIDs are generated at runtime; no `const` context would benefit. The `uuid!` macro handles compile-time parsing for the inner `Uuid`.

- **`get_version_num()` vs `get_version()`**: Using `get_version()` with `Version::SortRand` is more readable and type-safe than `get_version_num() == 7`. The "future-proof" argument for `get_version_num()` only matters for unknown versions, which we explicitly check against.
