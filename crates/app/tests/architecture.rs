...
mod tests {
    ...
    fn contexts_must_not_import_each_other() {
        ...
        for &ctx in &contexts {
            let pattern = format!("../../crates/{ctx}/src/**/*.rs");
    ...
    fn config_must_not_import_discovery_diagnostics_or_source_policy() {
        let files =
            glob("../../crates/config/src/**/*.rs").expect("read config glob");
    ...
    fn config_must_not_own_filesystem_candidate_discovery_modules() {
        assert!(
            !std::path::Path::new("../config/src/candidates.rs").exists(),
            "Config must not own filesystem candidate discovery"
        );
        assert!(
            !std::path::Path::new("../config/src/location.rs").exists(),
    ...
    fn builder_must_not_hardcode_system_global_config_path() {
        let content = fs::read_to_string("../config/src/builder.rs")
    ...
    fn check_imports(
        ...
        for &other in contexts {
            ...
            let import_pattern = format!("trace_{other}");
            assert!(
