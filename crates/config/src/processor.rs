...
impl<T: ConfigType> ConfigFileProcessor<T, Comparison, Unknown<T>> {
    ...
    ///
    /// ```rust,no_run
    /// use trace_config::processor::{ConfigFileProcessor, GlobalConfig};
    ///
...
mod tests {
    ...
    use super::*;
    use crate::{raw::RawLogging, views::RawFileVersion};
    ...
    fn global_config_compute_field_hashes_skips_none_fields_but_includes_present()
     {
        let mut raw = create_raw_global_config();
        raw.template = Some(crate::raw::RawTemplateConfig::default());
        let hashes = GlobalConfig::compute_field_hashes(&raw);
