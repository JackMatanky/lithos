...
fn hash_raw_global(raw: &crate::raw::RawGlobalConfig) -> Blake3Hash {
    let serialized = toml::to_string(raw).unwrap_or_default();
    Blake3Hash::compute(serialized.as_bytes())
}

fn hash_raw_vault(raw: &crate::raw::RawVaultConfig) -> Blake3Hash {
    let serialized = toml::to_string(raw).unwrap_or_default();
    Blake3Hash::compute(serialized.as_bytes())
}
...
///
/// ```ignore
/// use trace_config::views::{RawGlobalConfigView, RawFileVersion};
///
...
impl RawGlobalConfigView {
    ...
    ///
    /// ```ignore
    /// use trace_config::views::RawGlobalConfigView;
    ///
    /// let view = RawGlobalConfigView::new(
    ///     "/home/user/.config/lithos/config.toml".into()
    /// );
    ...
    #[inline]
    #[must_use]
    pub fn is_fresh(&self, raw: &crate::raw::RawGlobalConfig) -> bool {
    ...
    #[inline]
    #[must_use]
    pub fn matches_raw(&self, raw: &crate::raw::RawGlobalConfig) -> bool {
        self.is_fresh(raw)
    }
}
...
///
/// ```ignore
/// use trace_config::{vault::VaultId, views::{RawVaultConfigView, RawFileVersion}};
///
...
impl RawVaultConfigView {
    ...
    ///
    /// ```ignore
    /// use trace_config::{vault::VaultId, views::RawVaultConfigView};
    ///
    /// let view = RawVaultConfigView::new(
    ///     "/vault/.lithos/lithos.toml".into(),
    /// );
    ...
    #[inline]
    #[must_use]
    pub fn is_fresh(&self, raw: &crate::raw::RawVaultConfig) -> bool {
        self.latest_version().is_some_and(|latest| {
            let timestamp_match = raw.metadata.as_ref().is_some_and(|meta| {
                latest.is_timestamp_match(
                    meta.times().created_at(),
    ...
    #[inline]
    #[must_use]
    pub fn matches_raw(&self, raw: &crate::raw::RawVaultConfig) -> bool {
        self.is_fresh(raw)
    }
}
...
///
/// ```ignore
/// use trace_config::views::RawFileVersion;
/// use std::time::SystemTime;
///
