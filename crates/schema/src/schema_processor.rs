...
use crate::{
    ...
    error::{
        SchemaBuilderError, SchemaError, SchemaReadError, SchemaResolutionError,
    },
    ...
    views::{
        RawPropertyHashIndex, RawSchemaView, RawView as _, RawViewRead as _,
...
impl<T> GraphNode for ProcessorNode<T> {
    type Payload = T;
...
impl<T> GraphNodeMut for ProcessorNode<T> {
...
type FileState =
    (NewBatch<InitialRead>, HashMap<SchemaId, FoundPayload>, Vec<SchemaId>);
...
impl SchemaProcessor<Discovery, Review> {
    ...
    #[expect(
        ...
        reason = "Ordering is irrelevant for file state classification"
    )]
    pub(crate) fn from_discovery_result(
        discovery: DiscoveryResult,
    ...
    #[expect(
        ...
        reason = "internal helper threads discovery state and file source"
    )]
...
impl SchemaProcessor<FileParsed, Compared> {
    ...
    fn parse_new(
        ...
        for (id, read) in new_schemas.into_sorted_iter() {
            let schema_name = schema_stem_from_path(std::path::Path::new(
                read.path.as_str(),
            ))?;
...
impl SchemaProcessor<PropertyAnalysis, Graphed> {
    ...
    pub(crate) fn analyze_properties(
        ...
        for (id, node) in graph.graph().iter() {
            if node.relation().requires_merge() {
        ...
        let affected: HashSet<SchemaId> = if merge_roots.is_empty() {
            ...
            crate::inheritance::affected_subtree(graph.graph(), &merge_roots)
        };
            ...
            |id, node| -> Result<ProcessorNode<PipelinePayload>, SchemaError> {
                ...
                let (status, payload) = match node.payload {
                    PipelinePayload::Inheritance(InheritanceBranch::Fresh(
                        ...
                        let bank_changed = Self::bank_changed(
                            &payload.view,
                            property_bank_delta,
                        );
                        ...
                        if bank_changed {
                            ...
                            let version =
                                Self::build_version(&raw, content_hash)?;
                    ...
                    PipelinePayload::Inheritance(InheritanceBranch::New(
                        ...
                        let hashes = crate::views::HashRecord::new(
                            payload.content_hash,
                            property_hashes.into(),
                        );
                        let version = crate::views::SchemaVersion::new(
                            file_info,
                            hashes,
                            &payload.raw,
                        )?;
                    ...
                    PipelinePayload::Inheritance(
                        ...
                        if node_status == NodeStatus::StaleBankReferences {
                            ...
                            let excludes_delta = ExcludesDelta::from_slices(
                                payload.view.current().map_or(
                                    &[],
                                    crate::views::SchemaVersion::excludes,
                                ),
                            ...
                            let property_delta =
                                PropertyDeltaEngine::for_schema(
                                    &payload.raw,
                                    old_property_hashes,
                                )
    ...
    fn build_version(
        raw: &RawSchema,
        content_hash: Blake3Hash,
    ) -> Result<crate::views::SchemaVersion, SchemaError> {
        let property_hashes = raw.properties().compute_hashes();
        let file_info = raw.metadata().clone();
        let hashes =
            crate::views::HashRecord::new(content_hash, property_hashes.into());
        crate::views::SchemaVersion::new(file_info, hashes, raw)
    }
}
...
impl SchemaProcessor<Construction, Analyzed> {
    ...
    pub(crate) fn construct_schemas(
        ...
        use crate::expander::RefExpander;
        ...
        let update_ids: Vec<SchemaId> = rebuild_ids
            ...
            .filter_map(|id| {
                ...
                let extends_change = node.relation();
...
mod tests {
    ...
    use crate::{
        ...
        views::{
            HashRecord, RawPropertyHashIndex, RawSchemaView, SchemaVersion,
        },
    };
    ...
    fn make_view(name: &str, content_hash: Blake3Hash) -> RawSchemaView {
        ...
        let version = crate::views::SchemaVersion::new(metadata, hashes, &raw)
    ...
    fn deleted_nodes_pass_through_compare_and_parse() {
        ...
        assert!(matches!(
            compared_deleted.payload(),
            PipelinePayload::Deleted(_)
        ));

        let parsed = compared.parse().expect("parse succeeds");
    ...
    fn analyze_properties_rebuild_from_fresh_avoids_io() {
        ...
        // Write the file to disk so read_to_string succeeds
        let full_path = temp.path().join(path_str);
        std::fs::write(&full_path, "\"$version\" = \"1.0\"\n[properties]\n")
            .expect("write succeeds");

        // 1. Create a raw schema with a bank reference
        let bank_prop_name = PropertyName::try_new("affected").unwrap();
        let mut raw = make_raw_schema("schema");
        let mut props = std::collections::HashMap::new();
        let ref_path: crate::raw::property::RawPropertyRefPath =
            serde_json::from_str(&format!(
                "\"#property_bank/{bank_prop_name}\""
            ))
            .unwrap();
        props.insert(
            PropertyName::try_new("prop").unwrap(),
            crate::raw::property::RawProperty::Ref(
                crate::raw::property::RawPropertyRef {
                    ref_path,
                    required: None,
                    multi: None,
                    options: None,
        ...
        raw.properties = crate::raw::property::RawPropertyMap::from_map(props);

        // 2. Set metadata with a distinctive size to track it
        let metadata =
            FileMetadata::new(FsTimes::new(None, None), 12345, false);
        let hashes = HashRecord::new(
            Blake3Hash::new([7u8; 32]),
            RawPropertyHashIndex::default(),
        );
        let version = SchemaVersion::new(metadata.clone(), hashes, &raw)
            .expect("valid schema view fixture");
        let view = RawSchemaView::new(path.clone(), version);
        ...
        let processor = SchemaProcessor::<PropertyAnalysis, Graphed> {
            status: Graphed {
                graph: builder.build(),
                deleted_ids: Vec::new(),
            },
        ...
        // 3. Trigger a property bank change to force a rebuild transition
        let mut bank_delta = HashSet::new();
        bank_delta.insert(bank_prop_name);

        let bank = PropertyBank::default();

        let result = processor
            .analyze_properties(&source, &bank, Some(&bank_delta))
            .expect("analyze_properties succeeds");

        // 4. Assert that it used our metadata (12345) instead of disk metadata
        //    (1)
        let node = result.status.graph.graph().get(id).expect("node exists");
        let payload = node.payload().as_analysis().expect("is analysis");

        assert!(matches!(payload, AnalysisBranch::Rebuild(_)));
        if let AnalysisBranch::Rebuild(rebuild) = payload {
            assert_eq!(rebuild.metadata.size(), 12345);
        }
    }
}
