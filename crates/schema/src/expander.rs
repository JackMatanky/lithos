...
use super::{
    ...
    raw::{
        date::RawDateSpec, file::RawFileSpec, number::RawNumberSpec,
        property::RawPropertyRef, string::RawStringSpec,
    },
};
        ...
        match base {
            ...
            PropertySpec::String(spec) => {
                ...
                let overrides = RawStringSpec {
                    options: ref_entry.options.clone(),
    ...
    mod expand_property {
        ...
        fn ref_type_mismatch_returns_error() {
            ...
            assert!(
                matches!(
                    result,
                    Err(SchemaError::Builder(ref box_err)) if matches!(**box_err, crate::error::SchemaBuilderError::PropertyBuilder(
                        crate::error::PropertyBuilderError::OverridePropertyRefSpecTypeMismatch { .. }
                    ))
                ),
            ...
            assert!(
                matches!(
                    ...
                    Err(SchemaError::PropertyRef(
                        crate::error::PropertyRefError::TargetPropertyNotFoundInBank { .. }
                    ))
                ),
