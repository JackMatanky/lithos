...
use crate::{
    error::{SchemaError, StringSpecError, StringValueValidationError},
    raw::string::{
        RawOptions, RawStringPattern, RawStringProperty, RawStringSpec,
    },
};
    ...
    pub fn try_new(
            ...
            && opts.is_empty()
        {
            return Err(SchemaError::PropertySpec(
                StringSpecError::EmptyOptionsList.into(),
            ));
        }
        ...
        if let (Some(p), Some(opts)) = (spec.pattern(), spec.options()) {
            for opt in opts {
                if p.validate(opt.value()).is_err() {
                    return Err(SchemaError::PropertySpec(
                        StringSpecError::OptionValueViolatesPattern {
    ...
    pub(super) fn validate(&self, value: &str) -> Result<(), SchemaError> {
            ...
            && !entries.iter().any(|e| e.value() == value)
        {
            return Err(SchemaError::PropertyValue(
                StringValueValidationError::ValueNotInAllowedOptions {
                    value: value.into(),
...
impl TryFrom<RawStringSpec> for StringSpec {
    type Error = SchemaError;

    #[inline]
    fn try_from(raw: RawStringSpec) -> Result<Self, Self::Error> {
...
impl StringPattern {
    ...
    pub fn try_custom<S: Into<String>>(
        ...
        regex::Regex::new(&pattern).map_err(|e| {
            SchemaError::PropertySpec(
                crate::error::StringSpecError::InvalidCustomRegexPattern {
    ...
    pub fn validate(&self, value: &str) -> Result<(), SchemaError> {
        ...
        if !matches {
            return Err(SchemaError::PropertyValue(
                crate::error::StringValueValidationError::ValueViolatesPattern {
                    value: value.into(),
...
impl From<crate::raw::string::RawStringFormat> for StringPattern {
    #[inline]
    fn from(format: crate::raw::string::RawStringFormat) -> Self {
        use crate::raw::string::RawStringFormat;
        match format {
...
impl OptionEntries {
    ...
    pub fn try_new(entries: Vec<OptionEntry>) -> Result<Self, SchemaError> {
        if entries.is_empty() {
            return Err(SchemaError::PropertySpec(
                crate::error::StringSpecError::EmptyOptionsList.into(),
            ));
        }
...
impl OptionEntry {
    ...
    pub fn try_new<V: Into<String>>(
        ...
        if value.trim().is_empty() {
            return Err(SchemaError::PropertySpec(
                crate::error::StringSpecError::EmptyOptionValue.into(),
            ));
        }
        Ok(Self {
            value,
...
impl TryFrom<crate::raw::string::RawEntryValue> for OptionEntry {
    type Error = SchemaError;

    #[inline]
    fn try_from(
        raw: crate::raw::string::RawEntryValue,
...
mod tests {
        ...
        fn rejects_empty_entries() {
            ...
            assert!(matches!(
                ...
                Err(SchemaError::PropertySpec(
                    crate::error::PropertySpecError::String(
                        crate::error::StringSpecError::EmptyOptionsList
                    )
                ))
            ));
        }
    ...
    mod string_spec {
        ...
        use super::*;
        use crate::raw::string::{RawOptions, RawStringSpec};

        #[rstest]
        #[case::options_match(
            RawStringSpec {
                options: Some(RawOptions::Plain(vec![
                    "A".into(),
                    "B".into(),
                ])),
                ..Default::default()
        ...
        #[case::options_mismatch(
            ...
            Err(SchemaError::PropertyValue(
                crate::error::StringValueValidationError::ValueNotInAllowedOptions {
                    value: "C".into(),
        ...
        #[case::regex_match(
            RawStringSpec { pattern: Some(crate::raw::string::RawStringPattern::Custom(r"^\d+$".into())), ..Default::default() },
            "123",
            Ok(())
        )]
        ...
        fn try_new_rejects_inconsistent_options() {
            ...
            assert!(matches!(
                ...
                Err(SchemaError::PropertySpec(
                    crate::error::PropertySpecError::String(
                        crate::error::StringSpecError::OptionValueViolatesPattern { .. }
                    )
                ))
            ));
        }
