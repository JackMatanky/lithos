...
mod tests {
    ...
    use super::*;
    use crate::raw::number::RawNumberSpec;
    ...
    #[case::below_min(
        ...
        Err(SchemaError::PropertyValue(
            crate::error::NumberValueValidationError::ValueOutsideAllowedRange {
                value: -1.0f64,
                min: Some(0.0f64),
                max: Some(10.0f64)
            }.into()
    ...
    #[case::invalid_step(
        ...
        5.2f64,
        Err(SchemaError::PropertyValue(
            crate::error::NumberValueValidationError::ValueViolatesStepIncrement {
                value: 5.2f64,
                step: 0.5f64
            }.into()
        ))
        ...
        assert!(
            matches!(
                ...
                Err(SchemaError::PropertySpec(
                    crate::error::PropertySpecError::Number(
                        crate::error::NumberSpecError::MinGreaterThanMax { .. }
                    )
                ))
            ),
    ...
    fn number_spec_rejects_nan_value() {
        ...
        assert!(
            matches!(
                result,
                Err(SchemaError::PropertyValue(
                    crate::error::PropertyValueError::Number(
                        crate::error::NumberValueValidationError::NonFiniteNumber { .. }
                    )
                ))
            ),
    ...
    fn number_spec_rejects_non_finite_min_bound() {
        ...
        assert!(
            matches!(
                ...
                Err(SchemaError::PropertySpec(
                    crate::error::PropertySpecError::Number(
                        crate::error::NumberSpecError::NonFiniteConstraintValue { .. }
                    )
                ))
            ),
