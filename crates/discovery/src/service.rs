...
/// [`DiscoveryContext`](crate::context::DiscoveryContext).
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl DiscoveryServiceConfig {
    /// Constructs service configuration with explicit global directories.
    #[inline]
    #[must_use]
    pub fn with_global_directories(global_directories: Vec<DirPath>) -> Self {
        Self {
            global_directories,
            ..Self::default()
        }
...
impl DiscoveryService {
    ...
    #[inline]
    pub fn new(config: DiscoveryServiceConfig) -> Result<Self, DiscoveryError> {
        config.validate()?;
        Ok(Self {
            config,
        })
...
mod tests {
    ...
    fn placeholder_cache_root() -> CacheRoot {
        crate::location::tests::placeholder_cache_root()
    }
