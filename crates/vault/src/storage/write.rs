...
impl RedbRepository {
    fn remove_file_path_index(
        ...
        if let Some(path_key) = path {
            ...
            table.remove(DbPathKey::from(path_key))?;
        }
        Ok(())
    }
...
impl FileDeleteContext {
    ...
    fn load(tx: &WriteTx, file_id: FileId) -> Result<Self, DbError> {
        ...
        let reverse_path_table =
            tx.try_open_table(PATH_BY_FILE_ID.definition())?;
        let path =
            reverse_path_table.get(&file_id)?.map(|g| g.value().into_inner());
...
impl DirDeleteContext {
    ...
    fn load(tx: &WriteTx, dir_id: DirId) -> Result<Self, DbError> {
        ...
        let reverse_path_table =
            tx.try_open_table(PATH_BY_DIR_ID.definition())?;
        let path =
            reverse_path_table.get(&dir_id)?.map(|g| g.value().into_inner());
...
mod tests {
    ...
    mod upsert {
        ...
        fn file_creates_reverse_path_index() {
            use crate::storage::tables::PATH_BY_FILE_ID;
            ...
            let recovered_path: Result<Option<PathKey>, _> =
                repo.store.read(|tx| {
                    ...
                    Ok(table.get(&file.id())?.map(|g| g.value().into_inner()))
                });
        ...
        fn dir_creates_reverse_path_index() {
            use crate::storage::tables::PATH_BY_DIR_ID;
            ...
            let recovered_path: Result<Option<PathKey>, _> =
                repo.store.read(|tx| {
                    ...
                    Ok(table.get(&dir.id())?.map(|g| g.value().into_inner()))
                });
        ...
        fn file_overwrite_updates_reverse_index() {
            ...
            let recovered_path: Result<Option<PathKey>, _> =
                repo.store.read(|tx| {
                    ...
                    Ok(table.get(&id)?.map(|g| g.value().into_inner()))
                });
