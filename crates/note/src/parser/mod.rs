...
//!    into the final domain [`RawNote`](crate::raw::RawNote), applying routing
//!    policies (e.g., whether list-item tags are global).
//!
//! # Core Invariants
//!
...
use crate::{
    ...
    parser::{
        assemble::RawAssembler, context::ParserContext, lexical::ArtifactLexer,
        structure::DocTree,
    },
    raw::RawNote,
};
...
mod tests {
    ...
    fn should_extract_thematic_break() -> Result<(), NoteIngestError> {
        ...
        assert!(matches!(
            ...
            Some(crate::raw::RawSectionKind::ThematicBreak)
        ));
        Ok(())
    }
